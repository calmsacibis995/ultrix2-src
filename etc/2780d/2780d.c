#ifdef lint
static char *sccsid = "@(#)2780d.c	1.6	ULTRIX	10/3/86";
#endif
...
#include "../../sys/netbsc/bsc_proto.h"
#include "../../sys/netbsc/bsc.h" 
#include <time.h>
#include "rje.h" 
#include "../../sys/netbsc/bsc_messages.h"
	...
	char *arg;	/* argument for message */
{
	...
	if(login == (char *) 0) {
		if((pw=getpwuid(uid)) == NULL) {
			perr(sprintf(s,"can't find uid %d\n",uid));
			mail(rjemesg,"root");
			free(s);
			return;
		}
...
*/
		if(mail(rjemesg,login) < 0) {
			unlink(BADMAIL);
			perr(sprintf(s,"can't mail to %s\n",login));
			mail(rjemesg,"root");
		}
	free(s);
}
...
sprintf(datetime,"%02d%02d%02d%02d\0",tp->tm_mday,tp->tm_hour,tp->tm_min,tp->tm_sec);
	...
	dfname=sprintf(outputfname,"Rje%s",datetime);
