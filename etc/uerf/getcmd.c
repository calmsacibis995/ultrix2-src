...
#ifndef lint
static char sccsid[]  =  "@(#)getcmd.c	1.10   (ULTRIX)   12/16/86"; 
#endif  lint
...
*      |         |         |         |    |    |         |
*   +--+--+   +--+--+   +--+--+   +--+--+ | +--+--+   +--+---+
*   |make |   | get |   | es  |   | get | | | es  |   |  es  |
*   |tree |   |tree |   |open |   |tree | | |parse|   |select|
*   +--+--+   +--+--+   +--+--+   +--+--+ | +--+--+   +--+---+
*                                         |
*                                         |
*                                         |
...
static struct parm_struc *malloc();
...
while (--argc > 0)
    ...
    *++argv;
	...
	while (j < plen)
		...
		parm->next_parm = malloc(sizeof(*parm));
		parm = parm->next_parm;
		}
...
long   status;
static struct parse_tree *malloc();
...
if (( tree = get_tree(UE$FLG_n)) != UE$NULL)
    ...
    if (es$open(&uctx,ES$EVENT,ES$MAILBOX,NULL,
		    &ueis,&udis,&usds,&ucds,&uads) != ES$SUCC)
	{
...
*	-  Calls erm_sellect who validates selection criteria and
...
if ((tree = get_tree(UE$FLG_c)) != UE$NULL)
	...
	do
		...
		return(UE$ERR_INVPAR);
	    pars_node = es$parse(&status,"eventclass eq %d",pcode);
	    if (status != ES$SUCC)
		return(UE$ERR_INVPAR);
...
if ((tree = get_tree(UE$FLG_T)) != UE$NULL)
    ...
    pars_node = es$parse(&status,
		"devclass eq %d, %d, %d",dc$dstape,dc$mbtape,dc$ubtape);
...
if ((tree = get_tree(UE$FLG_D)) != UE$NULL)
    ...
    pars_node = es$parse(&status,
	"devclass eq %d, %d, %d",dc$dsdisk,dc$mbdisk,dc$ubdisk);
    if (status != ES$SUCC)		/* status from es$parse	*/
	return(UE$ERR_INVPAR);
    BLD_NODE (pars_node,ES$AND,flag_node);
    BLD_NODE (flag_node,ES$OR,sel_tree);
...
if ((tree = get_tree(UE$FLG_A)) != UE$NULL)
    ...
    pars_node = es$parse(&status, "devclass eq %d",dc$adapter);
    if (status != ES$SUCC)		/* status from es$parse	*/
	return(UE$ERR_INVPAR);
    BLD_NODE (pars_node,ES$AND,flag_node);
    BLD_NODE (flag_node,ES$OR,sel_tree);
...
if ((tree = get_tree(UE$FLG_M)) != UE$NULL)
	...
	do
		...
		return(UE$ERR_INVPAR);
	    pars_node = es$parse(&status,"devclass eq %d",pcode);
	    if (status != ES$SUCC)
		return(UE$ERR_INVPAR);
	    BLD_NODE (pars_node,ES$OR,flag_node);
	    }
	...
	{
	pars_node = es$parse(&status,
		"devclass eq %d, %d",dc$cpu, dc$memory);
	if (status != ES$SUCC)
	    return(UE$ERR_INVPAR);
	BLD_NODE (pars_node,ES$OR,flag_node);
	}
...
if ((tree = get_tree(UE$FLG_O)) != UE$NULL)
	...
	do
		...
		return(UE$ERR_INVPAR);
	    pars_node = es$parse(&status,"coarsesyndrome eq %d",pcode);
	    if (status != ES$SUCC)
		return(UE$ERR_INVPAR);
	    BLD_NODE (pars_node,ES$OR,flag_node);
	    }
	...
	{
	pars_node = es$parse(&status,"eventtype eq 109,200");
	if (status != ES$SUCC)
	    return(UE$ERR_INVPAR);
	BLD_NODE (pars_node,ES$OR,flag_node);
	}
...
tree   = get_tree(UE$FLG_s);
tree_r = get_tree(UE$FLG_r);
if ((tree != UE$NULL) || (tree_r != UE$NULL))
    {
    if ((tree != UE$NULL) && (tree_r != UE$NULL))
	return(UE$ERR_INCPAR);
    if (tree != UE$NULL)			/* s flag	*/
	(void)strcpy(pstring,"recordnumber eq    ");
    else
	{ 					/* r flag	*/
	(void)strcpy(pstring,"eventtype    eq    ");
	tree = tree_r;
	}
	...
	do
	    {
	        ...
	        {
	        strncpy(pstring+i,",",1);
	        strncpy(pstring+13,"range",5);
	        }
            pars_node = es$parse(&status,pstring);
            if (status != ES$SUCC)	/* status from es$parse */
	        return(UE$ERR_INVPAR);
	    BLD_NODE (pars_node,ES$OR,flag_node);
	    (void)strcpy(pstring+13,"eq   ");
	    }
        while ((parm = parm->next_parm) != UE$NULL);
        BLD_NODE (flag_node,ES$OR,sel_tree);
	}
...
if ((tree = get_tree(UE$FLG_H)) != UE$NULL)
	...
	do
	    {
            (void)strcpy(pstring,"hostname eq ");
            (void)strcat(pstring,parm->parm_string);
            pars_node = es$parse(&status,pstring);
            if (status != ES$SUCC)
	        return(UE$ERR_INVPAR);
...
if ((tree = get_tree(UE$FLG_t)) != UE$NULL)
    ...
    if ((parm = tree->parm_list) != UE$NULL)
	{
	    ...
	    if      ((strncmp(pstring,"s:",2)) == 0)
	        pars_node = es$parse(&status,"datetime ge %d",datetime);
	    else if ((strncmp(pstring,"e:",2)) == 0)
	        pars_node = es$parse(&status,"datetime le %d",datetime);
	    else 
	        return(UE$ERR_INVPAR);
	    if (status != ES$SUCC)
	        return(UE$ERR_INVPAR);
...
SELNODE *get_devtype(item_ptr,status)
...
static DD$STD_ITEM_DSD_PTR devtype_tbl;

static short item_cnt;
short next_item;
...
if (devtype_tbl == UE$NULL)
    {
    devtype_tbl = find_std_item_dsd(DD$devtype);
    if ((long)devtype_tbl == DD$UNKNOWN_ITEM)
	{
	*status = UE$ERR_DEVTBL;
	return (mult_node);
	}
    else
	item_cnt = devtype_tbl->COUNT;
    }
for (i = 0; i < len; i++)
    item[i] = toupper(item_ptr[i]);

for (next_item = 0; next_item < item_cnt; next_item++)
    {
    len_label = strlen(devtype_tbl->ITEM[next_item].LABEL);
    for (i = 0, j = 0; i < len_label; i++)
	{
	if (devtype_tbl->ITEM[next_item].LABEL[i] != ' ')
	    tbl_label[j++] =
		toupper(devtype_tbl->ITEM[next_item].LABEL[i]);
	}
    tbl_label[j] = '\0';
    if ((len >= 4) && (len != j))
	continue;
    if (strncmp(item,tbl_label,len) == 0)
	{
	pcode = devtype_tbl->ITEM[next_item].CODE;
	one_node = es$parse(&stat,"devtype eq %d",pcode);
	if (stat != ES$SUCC)
	    {
	    *status = UE$ERR_INVPAR;
	    return (mult_node);
