#ifndef lint
static char *sccsid = "@(#)vipw.c	1.2 (Ultrix) 12/9/85";
...
main ()
{
    ...
    while (fgets (buf, sizeof (buf) - 1, pwd_fd) != NULL)
	fputs (buf, tmp_fd);
	...
	switch (fn)
		...
		*/
		...
		if (*cp
		    ...
		    {
		    fprintf(stderr,
		    	"vipw: %s entry has an invalid shell: %s      %s\n",
			typename,
			cp,
			"Must be one of: /bin/sh /bin/csh");
