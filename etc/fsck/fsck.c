#ifndef lint
static char *sccsid = "@(#)fsck.c	1.11	ULTRIX	3/3/87";
#endif
 ...
 *
 *	Stephen Reilly, 30-Nov-84
 * 001- If the size of file is very large use howmany can result in
 *	negative number cause a segement fault
 *
 *	2.30 (Berkeley) 9/19/83";
 *
 * 2) - 15 Mar 85 -- funding	
 *	Added named pipe support (re. System V named pipes)
 *
 * 	Paul Shaughnessy (prs)
 * 003- Added multiple pass functionality to checkfilesys()
 *
 * 004 - 23 Oct 86 -  prs
 *	Added not writing clean byte if -n flag is set
 *
 * 005 - 16 Dec 86 - prs
 *	Fixed problem with fsck trying to check nfs file systems if no
 *	arguments were given. Also added functionality for user level
 *	fsck.
 *
 * 006 - 29 Jan 87 - prs
 *	Fixed problem with the primary super block not being rewritten if
 *	an alternate super block was specified.
 *
 * 007 - 03 Feb 87 - prs
 *	Added initializing of fs_optim in the alternate super block
 *	when verifying information in primary super block. fs_optim
...
#define NUMRETRIES	3	/* max tries a file system will be checked */
...
#define PORT	((dp->di_mode & IFMT) == IFPORT)		/* 2 */
...
gno_t	badlncnt[MAXLNCNT];	/* table of inos with zero link cnts */
gno_t	*badlnp;		/* next entry in table */
...
gno_t	imax;			/* number of inodes */
gno_t	lastino;		/* hiwater mark of inodes */
gno_t	lfdir;			/* lost & found directory */
	...
	char	*argv[];
{
	...
	nmountedfs = getmnt(&dufus, fs_data, MSIZE);
	if (argc) {
		while (argc-- > 0) {
			hotroot = 0;
			checkfilesys(*argv++);
	...
	do {
		...
		while ((fsp = getfsent()) != 0) {
			...
			if (strcmp(fsp->fs_type, FSTAB_RW) &&
				...
				continue;
			/*
			 * 005 - If fs_type is not a local file system,
			 * get next entry.
			 */
	...
	char *filesys;
{
	...
	/*
	 * 005 - If a user level mount is being performed, make sure
...
#define GROUP_EXEC 0000010
	...
	/*
	 * 005 - Get partition file info. ioctl system call can only
	 * have the raw device as an argument. Also, the device
	 * must be opened at least once to make sure partition
	 * table info is on disk.
	 */
	if ((sb.st_mode & GFMT) == GFBLK)
		raw = rawname(filesys);
	else
		raw = filesys;
	fso = creat(raw, 0666);
...
#define PART(dev) (minor(dev) % 8)
	...
	for (; retries > 0; retries--) {	/* 003 - Loop for NUMRETRIES or
		...
		case 1:
			break;
		default:
			fprintf(stderr,"Bad return from setup\n");
			exit(2);
		}
	...
	/* 5: check resource counts in cylinder groups */
		...
		if ((dfile.mod) | (sblk.b_dirty) | (fileblk.b_dirty) | 
			...
			/*
			 * 003 - If root was dirty, set rootdirty to 1.
			 */
			if (hotroot)
				rootdirty = 1;
			if (retries <= 1) {
...
#	define altsblock asblk.b_un.b_fs
	...
	if(only_when_needed && (sblock.fs_fmod == FS_CLEAN)) {
	...
	altsblock.fs_optim = sblock.fs_optim;		/* 007 */
		...
		for (i = 0; i < sblock.fs_ipg; i++, inumber++) {
			...
			if(inumber < ROOTINO) {
				if(dp->di_mode || dp->di_size || dp->di_nlink){
					char mess[50];
					sprintf(mess, "inode %d should be cleared, zap it",
					inumber);
					if(reply(mess) == 1) {
						zapino(dp);
						inodirty();
			...
			if (ALLOC) {
				...
				if (!preen && BADBLK &&
					...
					goto unknown;
				/*
				 *	This fix will prevent a very large
				 *	size from causing a segement fault
				 */
				if (dp->di_size < 0 ||		/* 001 */
...
ckfini()
{
	...
	if (sblk.b_bno != SBLOCK) {
		...
		flush(&dfile, &sblk);
		/*
		 * 006 - Turn bflag off. Next pass through, use the
		 * primary super block.
		 */
		bflag = 0;
	}
