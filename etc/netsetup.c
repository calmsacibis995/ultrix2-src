#ifndef lint
static	char	*sccsid = "@(#)netsetup.c	1.8  decvax!marc (ULTRIX)  2/12/87";
#endif lint
/************************************************************************
 *									*
 *			Copyright (c) 1984 by				*
...
char *if_table[] = 
{
	...
	"de",	/* DEUNA - UNIBUS */
	"qe",	/* DEQNA - Q-Bus  */
	"se",	/* DESVA - VAXstation 2000 and MicroVAX 2000 */
	"dmc",	/* Point to Point */
	0
};
...
char **argv;
{
	...
	if (stat(NETWORKS, &statb) == 0 && statb.st_size > 0)
		setupisdone = TRUE;
...
askagain:				
			if (if_default)
				prompt(buf, 512, "What is the device name of your Ethernet Interface [%s]? ", if_default);
			else
				prompt(buf, 512,"What is the device name of your Ethernet Interface? ");
			...
			while (*cp && *cp != ' ' && *cp != '\t' && *cp != '\n')
				cp++;
			*cp = '\0';
			if (yesno(YES, "\nYour Ethernet interface is \"%s\".  Is this correct", xp))
				break;
		}
		 ...
		 */
		sprintf(buf, "%s",inet_ntoa((long)broadaddr.s_addr & netmask.s_addr));
 ...
 *
 * Determine the ethernet network interface. Look in
