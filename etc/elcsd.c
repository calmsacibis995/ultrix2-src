#ifndef lint
static char *sccsid = "@(#)elcsd.c	1.12	ULTRIX	12/4/86";
#endif lint
...
#define MSIZE 	(sizeof(struct fs_data))
#define MISSED_CNT 3

struct fs_data totmountbuf;
struct fs_data *mountbuf = &totmountbuf;
char buf[10240]; 
char wbuf[8192]; 
	...
	char **argv;
{
	...
	if (!sglusrflg) {
	    ...
	    (void) stat(logpath, stp);
	    (void) setmarker(stp, &logp_marker);
	}
 ...
 *
 *  Description: Fill in trailer code, sid, and errlog version in
...
char *buffer;
{
	...
	while (cc > 0) {
	    ...
	    if (elrhdp->rhdr_time == 0) {
		elrhdp->rhdr_time = savetime;
	    }
	    elrhdp->rhdr_sid = pidsid.sid;
	    elrhdp->rhdr_elver[0] = elver[0];
	    elrhdp->rhdr_elver[1] = elver[1];
...
char *hname;
{
	...
	if (fd >= 0) {
	    ...
	    (void)fstat(fd, stp);
	    (void)setmarker(stp, &oftp->marker);
	}
	return(fd);
}
/*
 *  Function: setmarker(stp, marker_ptr)
 *
 *  Description: set marker entry in eloft to indicate entry in mount tbl
 *		 corresponding to stp
 *
 *  Arguments:  struct stat *stp	pointer to stat output 
 *		long *marker_ptr	ptr to eloft.marker
 *
 *  Return value: void
 *
 *  Side effects: none
 */
setmarker(stp, marker_ptr)
struct stat *stp;
long *marker_ptr;
{
	int ret = 1;
	u_int loc = 0;
	int found = 0;
	struct fs_data *gfd = mountbuf;;

	while (ret > 0  && !found) {
	    ret = getmnt(&loc,mountbuf,MSIZE);
	    if (ret > 0) {
	        if (gfd->fd_dev == stp->st_dev) {
		    *marker_ptr = --loc;
		    found = 1;
		}
	    }
...
/*
 *  Function: fsfull(p, stp)
 *
 *  Description: Check whether file system is almost full, return -1 if it is.
 *
 *  Arguments:  p		ptr to open file tbl entry, or 0
 *		stp		struct stat pointer
 *
...
struct stat *stp;
{
	...
	gret = getmnt(&marker,mountbuf,MSIZE);
        if (gret > 0) { 
            if (gfd->fd_dev == stp->st_dev) {
		signed_bfreen = gfd->fd_bfreen;
	        if ( (signed_bfreen < 0) || (FSPERCENT * gfd->fd_btot  / 100 > gfd->fd_bfreen) ) {
		        ret = -1;
		}
	   } 
	}
...
getdmpfil()
{
	...
	while ((cc = read(fd,(char *)&pos,sizeof(pos))) != 0) {
		...
		amt = ple - pout;
		if ((amt > 43) && (amt < 8180) && (pout > &buf[11])
			&& ((pout + amt) < &buf[8192])) {
		    fillinhdr(amt,pout);
		    if (remlogflg > 0)
			sndrem(amt,pout);
		...
		amt = pin - elbufp->kque; 	/* read data from kque to in */
		if ((amt > 43) && (amt < 8180)) {
		    fillinhdr(amt,elbufp->kque);
		    if (remlogflg > 0)
			sndrem(amt,elbufp->kque);
		    else
		        (void)wrtdata(LOCLOG,elbufp->kque,amt);
		}
	    }
	    else if (pin > pout) {
	    ...
	    if (pin > elbufp->sque) {
		amt = pin - elbufp->sque; 	/* read data from sque to sin */
		if ((amt > 43) && (amt < 2044)) {
		    fillinhdr(amt,elbufp->sque);
		    if (remlogflg > 0)
			sndrem(amt,elbufp->sque);
		    else
		        (void)wrtdata(LOCLOG,elbufp->sque,amt);
		}
	    }
	}
	(void)close(fd);
...
char *cp2;
{
	...
	if (!sglusrflg && logfp != NULL) {
		...
		if (missed_cnt < MISSED_CNT) {  /* not too many to syslog */
		    ...
		    fprintf(logfp,"%s\n",line);
	            (void)fflush(logfp);
	            (void)sprintf(line,"elcsd:%s %s; %%m",elcsmsg[67],logpath);
		...
		if (missed_cnt < MISSED_CNT ) {  /* don't log too many */
		    errno = saverrno;
	            (void)sprintf(line,"elcsd:%s %s; %%m",cp1,cp2);
	            syslog(LOG_SALERT, line);
		}
	    }
	}
