#ifndef lint
static char *sccsid = "@(#)nfs_umount.c	1.2	ULTRIX	2/13/87";
#endif lint
 ...
 * client crash.  The routine "eachreply" is upcalled from clnt_broadcast
 * for each reply that is received.
 */
...
unmountall()
{
	...
	pid = fork();
	if (pid < 0)
		perror("Warning: nfs_umount couldn't fork:");
	if (pid == 0) {
