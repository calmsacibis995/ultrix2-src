...
#ifndef lint
static	char	*sccsid = "@(#)snapcopy.c	1.1	(ULTRIX)	4/10/86";
#endif lint
 ...
 *		     returned from the front end is printed and the	*
 *		     snapshot files are NOT invalidated.		*
 *									*
 ************************************************************************/
...
#ifndef DEBUG
	     putc(INVAL_1,dev_fp); 	/* invalidate snap1 */
#endif
	     system(rename_1);
	    }
}
...
void copy_2()			/* copy snap2 then invalidate it */
{
	...
	if ((system(COPY_2)) != 0)
		...
		fprintf(stderr,"snapcopy: can't write to error log\n");
#ifndef DEBUG
	     putc(INVAL_2,dev_fp);	/* invalidate snap2 */
#endif
	     system(rename_2);
	    }
}
...

{
	...
	sprintf(message_1,"%02d:%02d:%02d-snap1.dat created in %s",
		tp->tm_hour, tp->tm_min, tp->tm_sec, argv[1]);
	sprintf(message_2,"%02d:%02d:%02d-snap2.dat created in %s",
		tp->tm_hour, tp->tm_min, tp->tm_sec, argv[1]);

	snap_stat = 0;

	if (argc != 2) 	/* check for correct arguement count */
		{
		 printf("\nUsage: snapcopy directory\n");
		 exit(ERROR);
	        }
	...
	if ((stat(argv[1],&stat_buf)) != 0)	/* is there such a place */
	       {
