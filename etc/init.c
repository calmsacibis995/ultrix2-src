...
#ifndef lint
static char *sccsid = "@(#)init.c	1.14    ULTRIX  1/29/87";
#endif lint
/*
    init
	...
	char **argv;
{
	...
	int i;
	char buf[1024];
/* coded /upgrade in ascii plus a constant 0x64 */
static char fpdata[] = {
	0x93,
	0xd9,
	0xd4,
...
char fpstring[256];
	...
	if((fd=open(fpstring,O_RDONLY)) >= 0)
	{
		...
		{
		n=buf[87];
		if(buf[87]==((~buf[88])-5) && (buf[87] == (~buf[87+n+9])))
			x=n;
		}
	}
	close(fd);
	quota(Q_SLIMIT,0,x,0);
	time0 = time(0);
#ifdef vax
	howto = r11;
#else
	...
	char *arg;	/* last argument on line */
{
	...
	sprintf(buf, "init: exec failed: cmd= %s, %s", argv[0], argv[1]);
