...
# ifndef lint
static char *sccsid = "@(#)utilities.c	1.5	ULTRIX	10/3/86";
# endif not lint
...
/* ------------------------------------------------------------------------
 * Modification History: /usr/src/etc/restore/utilities.c
 *
 *  8 Sep 86 -- fries
 *	Changed reply to query(for statchk.c compatibility)
 *	
 * 13 Feb 86 -- fries
 *	Changed messages to report disk device if disk is on input
 *	device.
 *
 * ------------------------------------------------------------------------
 */
	...
	char *name;
{
	...
	if (ino == 0 || BIT(ino, dumpmap) == 0){
		fprintf(stderr, "%s is not on ", name);
		switch(devtyp){
		  case TAP:
		   fprintf(stderr, "tape\n");
		   break;
...
onintr()
{
	if (query("restore interrupted, continue") == FAIL)
		done(1);
