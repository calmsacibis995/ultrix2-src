...
# ifndef lint
static char *sccsid = "@(#)dirs.c	1.8	ULTRIX	10/3/86";
# endif not lint
...
/* ------------------------------------------------------------------------
 * Modification History: /usr/src/etc/restore/dirs.c
 *
 *  8 Sep 86 -- fries
 *	Changed reply to query(statchk.c compatibility)
 *
 * 13 Feb 86 -- fries
 *	Changed messages to output disk messages if input device is
 *	a disk device.
 *
 * ------------------------------------------------------------------------
 */
	...
	int genmode;
{
	...
	switch(devtyp){
	 case TAP:
	   ...
	   break;
	 case DSK:
	   vprintf(stdout, "Extract directories from disk\n");
	   break;
	 case FIL:
	   vprintf(stdout, "Extract directories from file\n");
	   break;
	}
			...
			if (i == 0){
				switch(devtyp){
				  case TAP:
				   ...
				   break;
				  case DSK:
				   panic("Root directory is not on disk\n");
				   break;
				  case FIL:
				   panic("Root directory is not in file\n");
				   break;
				}
			}
			return;
	...
	daddr_t seekpt;
{
	...
	if (mf == NULL)
		return(itp);
	node.ino = ino;
	node.timep[0] = dip->di_atime.tv_sec;
	node.timep[1] = dip->di_mtime.tv_sec;
