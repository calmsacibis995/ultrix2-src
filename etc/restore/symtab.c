# ifndef lint
static char *sccsid = "@(#)symtab.c	1.4	(ULTRIX)	5/21/86";
# endif not lint
...
/* ------------------------------------------------------------------------
 * Modification History: /usr/src/etc/restore/symtab.c
 *
 * 13 Feb 86 -- fries
 *	Changed messages to display disk if disk device on input
 *
 * ------------------------------------------------------------------------
 */
			...
			if (hdr.dumpdate < dumptime){
				switch(devtyp){
				 case TAP:
				   ...
				   break;
				 case DSK:
				   fprintf(stderr, "Incremental disk too low\n");
				   break;
				}
			}
			else{
				switch(devtyp){
				 case TAP:
				   ...
				   break;
				 case DSK:
				   fprintf(stderr, "Incremental disk too high\n");
				   break;
				}
			}
			done(1);
