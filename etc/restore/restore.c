# ifndef lint
static char *sccsid = "@(#)restore.c	1.3	(ULTRIX)	5/21/86";
# endif not lint
...
/* ------------------------------------------------------------------------
 * Modification History: /usr/src/etc/restore/restore.c
 *
 * 13 Feb 86 -- fries
 *	Changed messages to report disk if disk device is on input.
 *
 * ------------------------------------------------------------------------
 */
	...
	int type;
{
	...
	if (BIT(ino, dumpmap) == 0) {
		vprintf(stdout, "%s: not on the ", name);
		switch(devtyp){
		  case TAP:
		   vprintf(stdout, "tape\n",0);
		   break;
	...
	char *symtabfile;
{
	...
	while (curfile.ino < maxino) {
		...
		while (first < curfile.ino) {
			...
			if (ep == NIL)
				panic("%d: bad first\n", first);
			fprintf(stderr, "%s: not found on ", myname(ep));
			switch(devtyp){
			  case TAP:
			   fprintf(stderr, "tape\n",0);
			   break;
		...
		if (ep == NIL){
			switch(devtyp){
			  case TAP:
			   ...
			   break;
			  case DSK:
			   panic("unknown file on disk\n");
			   break;
			  case FIL: 
			   panic("unknown file in dump file\n");
			   break;
			}
		}
		if ((ep->e_flags & (NEW|EXTRACT)) == 0){
			switch(devtyp){
			  case TAP:
			   ...
			   break;
			  case DSK:
			   badentry(ep, "unexpected file on disk");
			   break;
			  case FIL: 
			   badentry(ep, "unexpected file in dump file");
			   break;
			}
		}
		/*
