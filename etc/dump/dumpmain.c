...
# ifndef lint
static char *sccsid = "@(#)dumpmain.c	1.18	(ULTRIX)	1/9/87";
# endif not lint
	  ...
	  if ((statbfp = rmtstat(tape)) !=  NULL){
		...
		if (((statbfp->st_mode & S_IFMT) == S_IFBLK)
			...
			if (devgetp->category == DEV_TAPE){
			 ...
			 /* If TK50 or TK70, then set tape length */
			 if (((strcmp(devgetp->device, DEV_TK50)
				 == 0)||
			 (strcmp(devgetp->device, DEV_TK70)
				 == 0)) && (!tsize))
		            tsize =  1200L*120L;
			 }
			else {
		...
		if(((statbf.st_mode & S_IFMT) == S_IFBLK)
		   ||((statbf.st_mode & S_IFMT) == S_IFCHR)){
			...
			if (mt_info.category == DEV_TAPE){
			   ...
			   /* If TK50 or TK70, then set tape length */
			 if (((strcmp(mt_info.device, DEV_TK50)
				 == 0)||
			 (strcmp(mt_info.device, DEV_TK70)
				 == 0)) && (!tsize))
		              tsize =  1200L*120L;
			}
			else {
...
ignore:
	...
	emsg("This dump will occupy %ld (%d byte) blocks on %3.2f tape(s).\n", esize/ntrec,ntrec
* TP_BSIZE, fetapes);
	}

	/* Else if output is to a disk */
	else if (devtyp == DSK){
	emsg("Estimated %ld (%d byte) blocks on %3.2f disk(s).\n", esize/ntrec,ntrec * TP_BSIZE, fetapes);
	}
