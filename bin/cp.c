#ifndef lint
static char *sccsid = "@(#)cp.c	1.3	ULTRIX	10/3/86";
#endif
...
{
	int fold, fnew, n;
	char *last, destname[BSIZE], buf[BSIZE], *res;
	...
	if((stfrom.st_mode & S_IFMT) == S_IFDIR) {
		if (rflag) {
			...
			fprintf(stderr, "cp: %s is a dir and -r not specified\n",
			(res = (rindex(to, '/')) + 1) == (char *)1 ? to : res);
			return(1);
		}
	}
	if (stat(to, &stto) >= 0) {
