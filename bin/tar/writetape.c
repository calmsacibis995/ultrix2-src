...
# ifndef lint
static char *sccsid = "@(#)writetape.c	1.7	(ULTRIX)	1/15/87";
# endif not lint
...
if (!NFLAG) {
	...
	if (lastgid != sp->st_gid && (gp=getgrgid(sp->st_gid))) {
		char	*cp;
		cp = gp->gr_name;
...
/**/
		...
		if(doingasync) {
			...
			for(i=0, nleft--; i<doingasync && nleft>0; i++) {
				int bytes;
				if(pending[curptr] == -1) {
				mtops.mt_op = MTCSE;
				mtops.mt_count = 1;
