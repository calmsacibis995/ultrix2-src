#ifndef lint
static char sccsid[] = "@(#)run.c	1.4	ULTRIX	3/31/87";
#endif
...
*
*	10/10/85  David Metsky
* 001 - Rewrote the routine 'format' to be more robust.  It no longer
*	crashes when given precision or fieldwidth variables and it
*	now handles incorrect printf statements with more error
*	messages.  Also fixed spr ica-933 about changing records in
*	a line and then writing it out right away.
*
*	12/16/86 David Metsky
* 002 - Made sure that getsval is called every time in the forth section
*	of printf during calls to Xsprintf.  This is to fix a problem 
*	printing the NR variable and probably all pre-defined variables.
*
*	3/30/87 David Metsky
* 003 - Added two if statements that were removed by accident in version
*	1.3.
*
*************************************************************************/
			...
			switch(*s){
				...
				case '*':	/* precision or field width */
					*t++ = *s++;	/* store '*' */
					...
					if (!VarPrecision){
					    if (VarFieldWidth)
						error(FATAL,"More than one fieldwidth definition in printf(%s)",os);
					    VarFieldWidth = TRUE;
					    if (a == NULL)
				...
				default:
					*t = '\0';
					if (t >= fmt + sizeof(fmt))
						printf("format item %.20s... too long", os);
					endloop = FALSE;
					break;
			}
