 ...
 *
 * 	Rich Phillips, 26-July-1984
 * 001-	Don't issue a warning for byte or word displacements to global symbols
 *	defined in the current .s file. Also, put out relocation records
 *	when the referenced symbols are external so the loader can try
 *	to resolve them. 
 *
 ***********************************************************************/
#ifndef lint
	...
	for (i=0; i<n; i++,ap++) {/* now for the arguments */
		...
		case AEXP: /* expr */
			...
			if (argtype == A_BB) {
				...
				/* RAP001
					Issue the warning only if the symbol was
					not defined in this .s file.
				*/
				if ( (xp->e_xtype & XXTRN) && xp->e_xvalue==0)
				...
				if (!ISBYTE(argtype))
					yyerror("%s: Branch too far(%db): try -J flag",
						...
						argtype);
				/*RAP001
					If the symbol is external, let outrel
					handle things so relocation info is
					included in the .o file for this ref.
				*/
				if ( xp->e_xtype & XXTRN){
					reloc_how = TYPB + RELOC_PCREL;
					goto skipoutb;
					}
				break;
			}
			if (argtype == A_BW) {
				argtype = xp->e_xvalue - dotp->e_xvalue - 2;
				/*RAP001
					Don't issue the warning if the symbol
					is defined in this .s file.
				*/
				if ( (xp->e_xtype & XXTRN) && xp->e_xvalue==0)
					yywarning("%s: destination label is external",
						FETCHNAME(ITABFETCH(opcode)));
				if (!ISWORD(argtype))
					yyerror("%s: Branch too far(%db): try -J flag",
						...
						argtype);
				/* RAP001
					Set for call to outrel if the symbol is
					external, otherwise just put out the
					pc offset to the symbol.
				*/
				if ( xp->e_xtype & XXTRN)
					reloc_how = TYPW + RELOC_PCREL;
				else    {
					int offset = argtype;		
					outs( (char*)(&offset),2);
					}
