 ...
 *									*
 *									*
 *	001 - Modified yukkyexpr() so that if val == NAME, 		*
 *	      locxp->e_name *always* contains a pointer to a namelist   *
 *	      (used to return nil if it had been seen before, and	*
 *	      assigned e_value instead).				*
	...
	reg	np;
{
	...
	case NAME:
		...
		/* Forward reference? */
		if (( ((struct symtab *)np)->s_type&XTYPE)==XUNDEF) { 
			locxp->e_xvalue = 0;
			if (passno==1)
				((struct symtab *)np)->s_type |= XFORW;
		/* No, so just get the value */
		} else {
