static char *sccsid = "@(#)doname.c	1.4	ULTRIX	10/3/86";
#include "defs"
...
int ign, nopr;
char **stringp = &string;
char *string_end, **string_endp;

string_end = &string[outmax];
string_endp = &string_end;
++ndocoms;
if(questflag)
	return(NO);
...
if(touchflag)
	...
	{
 	subst(q->shbp,stringp,stringp,string_endp);
	string = *stringp;
	string_end = *string_endp;
	outmax = string_end - string;
