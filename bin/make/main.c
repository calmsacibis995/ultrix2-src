static char *sccsid = "@(#)main.c	1.4	ULTRIX	10/3/86";
# include "defs"
/*
...
*/
extern char *yytext,*templin,*string;
	...
	if (argv[i]!=0 && argv[i][0]=='-') {
		for (j=1 ; (c=argv[i][j])!='\0' ; ++j) {
			*op++ = c;
			...
			if((yytext = calloc(INMAX,sizeof(char))) == NULL){
			    fatal("Main: Out of memory.\n");
			}
			if((templin = calloc(INMAX,sizeof(char))) == NULL){
			    fatal("Main: Out of memory.\n");
			}
			if((string = calloc(OUTMAX,sizeof(char))) == NULL){
			    fatal("Main: Out of memory.\n");
			}
