%{#include "defs"
static	char *sccsid = "@(#)gram.y	1.4	ULTRIX	10/2/86";
%}
...
static char *yytextl;
char *text, *newtext;
char *templinp,**templinpp;
char *end_temp,**end_tempp;
register char c;
register char *p, *t;
char lastch, *lastchp;
...
yytextl = &yytext[yymax];
templinpp = &templin;
end_temp = &templin[tempmax];
...
if(fin == NULL)
	...
	for(p = text = yytext ;; *p++ = kc){
		if(p == yytextl - 1){
		    if((newtext = realloc(yytext, 2*yymax)) == NULL)
		    	fatal("Out of memory in nextlin\n");
		    yymax *= 2;
		    p = &newtext[p - yytext];
		    yytext = newtext;
		    text = yytext;
...
subst(yytext, templinpp, templinpp, end_tempp);
templin = *templinpp;
end_temp = *end_tempp;
tempmax = end_temp - templin;
