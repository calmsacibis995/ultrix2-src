#ifndef lint
static char *sccsid = "@(#)ld.c	1.9	ULTRIX	10/3/86";
#endif not lint
 ...
 *									*
 * 008  Victoria Holt, 01-Jul-86					*
 *	Now output 3 N_NSYMS records, using stabcnt.n_value field only,	*
 *	to avoid possible overflow.					*
 *									*
 * 007	Victoria Holt, 23-Dec-85					*
 *	Bug fix:  increment nfun counter for local routines (see	*
 *	vjh007 in source).						*
 *									*
 * 006  Robert Rodriguez, 15-Sep-85                                     *
 *	Add 4.3 speed ups                                               *
 *									*
 * 005  Victoria Holt, 29-Aug-85					*
 *	Big fix:  ld output N_NSYMS record when -x switch was specified.*
 *	Harmless, but shouldn't have been there by definition of the	*
 *	switch.								*
 *									*
 * 004  Victoria Holt, 14-Jun-85					*
 *	ld will now count the number of functions, source lines, and	*
 *	source files; this information is output in a stab record of	*
 *	type N_NSYMS for use by dbx.  It must be the first stab entry	*
 *	in the symbol table.						*
 *									*
 *	David L Ballenger, 17-May-1985					*
 * 003	Fix for QPR-00078.  The -H option did not work properly.  The	*
 *	size argument was being used to increase the base address for	*
 *	the data section, but the textsize written to the a.out header	*
 *	was not increased accordingly.					*
 *									*
 * 002- David L Ballenger, 07-Sep-84					*
 *	Fix error with definiton of "position" formal parameter for	*
 *	load2td function.						*
 *									*
 *	Stephen Reilly, 15-Feb-84					*
 * 001- Fix some dereferencing of NULL pointers				*
 *									*
 ************************************************************************/
...
/* Number of functions (estimate), number of source lines, and
 * number of source files.  This information is provided for dbx.
 * and is used when gflag is true (when linking -lg). (vjh)
 */
unsigned long nsline;
unsigned long nfun;
unsigned long nsol;
...
/* Initialize to be less than b_bufsize initially, to boundary align in file */
	...
	char	*b_buf;			/* Pointer to the buffer */  /*006*/
	int	b_bufsize;		/* Size of the buffer */     /*006*/
...
delexit()
{
	 ...
	 */
	fstat(biofd, &stbuf);
	size = round(stbuf.st_size, pagesize);		/*006*/
...
ldrand()
{
	...
	for (tp = tab; tp <= tplast; tp++) {
		...
		/*
		 *  Check to make sure that the symbol pointer is
		 *  valid
		 */
	...
	off_t loc;
{
	...
	while (text.size > 0) {
		...
		/* Count the number of source lines and source files
		 * found in the symbol table.  This information is used by
		 * dbx.  Unnamed blocks are counted as functions, but
		 * real procedures are counted from the loader namelist
		 * (N_TEXT+N_EXT, and N_TEXT symbols).
		 */
		if (type & N_STAB) {
		    switch (type) {
		        case N_LBRAC:
			case N_RBRAC:	/* unnamed blocks go in the functab */
			    nfun++;
			    break;

		        case N_SLINE:	/* source lines */
			    nsline++;
			    break;

			case N_SO:	/* source files with stab info */
			case N_SOL:	/* textual include files */
			    nsol++;
			    break;
		...
		if ((type&N_EXT)==0) {
			if (Xflag==0 || type & N_STAB ||
				cursym.n_un.n_name[0]!='L'  )	/* SLR001 */
				nlocal += sizeof cursym;
			if (type == N_TEXT) nfun++;	/* vjh007 */
			continue;
		}
		symreloc();
		if (enter(lookup()))
...
middle()
{
	...
	tsize += hsize ;	/* Increase by size given with -H option */
...
setupout()
{
	...
	tout = &toutb;						/*006*/
	bopen(tout, 0, stbuf.st_blksize);			/*006*/
	bwrite((char *)&filhdr, sizeof (filhdr), tout);
	if (zflag)						/*006*/
		bseek(tout, pagesize);				/*006*/
	wroff = N_TXTOFF(filhdr) + filhdr.a_text;
	outb(&dout, filhdr.a_data, stbuf.st_blksize);		/*006*/
	if (rflag) {
		outb(&trout, filhdr.a_trsize, stbuf.st_blksize); /*006*/
		outb(&drout, filhdr.a_drsize, stbuf.st_blksize); /*006*/
	}
	if (sflag==0 || xflag==0) {
		outb(&sout, filhdr.a_syms, stbuf.st_blksize);	/*006*/
		wroff += sizeof (offset);
		outb(&strout, 0, stbuf.st_blksize);		/*006*/
		if (gflag) {
		    outstabcnt();
		}
	}
...
top:
	...
	if (n > p_blksize) {					/*006*/
		take = n - n % p_blksize;			/*006*/
		lseek(infil, (sp->bno+1)<<p_blkshift, 0);	/*006*/
		...
		sp->size -= take;
		sp->pos += take;
		dseek(sp,(sp->bno+1+(take>>p_blkshift))<<p_blkshift,-1); /*006*/
		goto top;
	}
	*loc++ = get(sp);
	--n;
		...
		if (p->nuser==0 || (p = &page[0])->nuser==0) {
			if (page[0].nuser==0 && page[1].nuser==0)
				if (page[0].bno < page[1].bno)
					p = &page[0];
			p->bno = b;
			lseek(infil, loc & ~(long)p_blkmask, 0);	/*006*/
			if ((n = read(infil, p->buff, p_blksize)) < 0)	/*006*/
				n = 0;
			p->nibuf = n;
		} else
...
STREAM *asp;
{
	...
	if ((sp->nibuf -= sizeof(char)) < 0) {
		dseek(sp, ((long)(sp->bno+1)<<p_blkshift), (long)-1);	/*006*/
...
char *acp;
{
	...
	c = stb.st_blksize;					/*006*/
	if (c == 0 || (c & (c - 1)) != 0) {			/*006*/
		/* use default size if not a power of two */	/*006*/
		c = BLKSIZE;					/*006*/
	}							/*006*/
	if (p_blksize != c) {					/*006*/
		p_blksize = c;					/*006*/
		p_blkmask = c - 1;				/*006*/
		for (p_blkshift = 0; c > 1 ; p_blkshift++)	/*006*/
			c >>= 1;				/*006*/
		if (page[0].buff != NULL)			/*006*/
			free(page[0].buff);			/*006*/
		page[0].buff = (char *)malloc(p_blksize);	/*006*/
		if (page[0].buff == NULL)			/*006*/
			error(1, "ran out of memory (getfile)");/*006*/
		if (page[1].buff != NULL)			/*006*/
			free(page[1].buff);			/*006*/
		page[1].buff = (char *)malloc(p_blksize);	/*006*/
		if (page[1].buff == NULL)			/*006*/
			error(1, "ran out of memory (getfile)");/*006*/
	}							/*006*/
...
symreloc()
{
	...
	case N_EXT+N_TEXT:
		cursym.n_value += ctrel;
		/* Count the number of functions in the symbol table.
		 * This information is supplied for dbx.
		 */
		nfun++;
		return;
...
bopen(bp, off, bufsize)					/*006*/
	register struct biobuf *bp;			/*006*/
{

	bp->b_ptr = bp->b_buf = malloc(bufsize);	/*006*/
	if (bp->b_ptr == (char *)0)			/*006*/
		error(1, "ran out of memory (bopen)");	/*006*/
	bp->b_bufsize = bufsize;			/*006*/
	bp->b_nleft = bufsize - (off % bufsize);	/*006*/
	bp->b_off = off;
	bp->b_link = biobufs;
	biobufs = bp;
}
...
top:
	...
	if (cnt >= bp->b_bufsize) {			/*006*/
		if (bp->b_ptr != bp->b_buf)
			bflush1(bp);
		put = cnt - cnt % bp->b_bufsize;	/*006*/
		if (boffset != bp->b_off)
...
bseek(bp, off)							/*006*/
	register struct biobuf *bp;				/*006*/
	register off_t off;					/*006*/
{								/*006*/
	bflush1(bp);						/*006*/
								/*006*/
	bp->b_nleft = bp->b_bufsize - (off % bp->b_bufsize);	/*006*/
	bp->b_off = off;					/*006*/
}								/*006*/
