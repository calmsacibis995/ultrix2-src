static	char *sccsid = "@(#)pr.c	1.4	(ULTRIX)	9/30/85";
/*
 ...
 *
 *	Armando Stettner, April 5, 1984
 * 001- Added the -b switch.
 *
 ***********************************************************************/
...
char **argv;
{
	...
	for (nfdone=0; argc>1; argc--) {
		...
		if (**argv == '-') {
			...
			case 'b':		/* print blank page headers */
				nbflg++;
				continue;
			...
			default:
				if (numeric(*argv)) {	/* # of cols */
					...
					fprintf(stderr, "Usage: pr [-htbflwsm] [filename]\n");
					exit(1);
				}
				continue;
			}
