#ifndef lint
static char *sccsid = "@(#)ed.c	1.2 (Ultrix) 2/13/86";

/* Orig ID: "@(#)ed.c	1.1	9/14/84" */
#endif
/* (sysV)  ed.c	 1.23  */
/*
    ed	- SYS V  Version

    The assembly code for ed.c should be altered making the
    .data's for the following array .text's so that it can be
    shared (a la the Shell). Use 'edfun' to accompish this.

 ------------
 Modification History
 ~~~~~~~~~~~~~~~~~~~
02	12-Dec-85, Greg Tarsa
	Changed return code convention to match the V7 ed.
	This is to match vi, ex, and other editor return code
	conventions.  Perhaps the return code should be changed
	for all our editors, but this will have to be done in
	the V2 timeframe.

01	30-Jul-85, John Dustin
	Added 'W', append to an existing file, if not encrypted.  
...
struct	Fspec	{
	char	Ftabs[22];
	char	Fdel;
	char	Flim;	/* JSD: cc complains about 'misplaced unsigned' here */
	char	Fmov;
	char	Ffill;
};
struct	Fspec	fss;
...
/* Verify that line just read IS an encrypted file. */
			...
			while(fp < &funny[chcount])
				if(*fp++ & 0200)crflag = 1;
/* If it is encrypted, & -x option was used, & key is not null, decode it. */
#ifdef FSPEC
		...
		switch(*fsp) {
			...
			case '<':       if(havespec) return(-1);
					if(*(fsp+1) == ':') {
						havespec = 1;
						clear(f);
						/* if(!ioctl(1, TCGETA, &arg) && ((arg.c_oflag&TAB3) == TAB3)) JSD: sysV ioctl */
