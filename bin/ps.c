# ifndef lint
static char *sccsid = "@(#)ps.c	1.7	(ULTRIX)	10/21/85";
# endif not lint
...
getu()
{
	...
	/* pcbpf is the 1st page of the user page table */
	/* pstat will use this to seek to to get all the pte's of the u area */
	/* essentially pstat has to do the rest of this routine in order to */
	/* get the user structure */
	klseek(mem,(long)ctob(pcbpf)-(UPAGES+CLSIZE)*sizeof(struct pte),0);
