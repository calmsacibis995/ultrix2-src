#ifndef lint
static	char	*sccsid = "@(#)cc.c	1.10	(ULTRIX)	6/20/86";
#endif
...
int	sys_V_flag = 0;		/* System V environment DLB007 */
	...
	char **argv;
{
	...
	/* Check the SYSTYPE environment variable to see if we should
	 * be using the System V environment.  (DLB007)
	 */
	...
	for (i = 1; i < argc; i++) {
		...
		case 'b':
			...
			/*
			 *  This flag dictates that the user does not want
...
nocom:
	if (cflag==0 && nl!=0) {
		...
		if (gflag || Gflag)
			av[na++] = "-lg";
		if (proflag) {
			if (sys_V_flag) av[na++] = "-lcV_p" ; /* DLB007 */
			av[na++] = "-lc_p";
			 ...
			 *
			 * If sys_V_flag is set use the System V libraries
			 * in addition to the regular libraries. (DLB007)
			 */
			if (sys_V_flag)
