#ifndef lint
static	char	*sccsid = "@(#)output.c	1.3	(ULTRIX)	11/25/85";
#endif lint
	...
	register unsigned long	value;
{
	...
	case 2:
		...
		*--ptr = '\0';
		...
		return(ptr);
	case 8:
		return(sprintf(ptr,"%l#o",value));
	case 10:
		return(sprintf(ptr,"%ld",value));
	case 16:
		return(sprintf(ptr,"%lx",value));
	}
}
	...
	L_INT		o;
{
	...
	if (o < 0)
		 ...
		 * by a minus sign.
		 */
		return(sprintf(buffer,format,-o));
	else
		/* Positive value, so print value as is without the
		 * minus sign.
		 */
		return(sprintf(buffer,&format[1],o));
}
	...
	DO  IF c!='%'
	    ...
	    ELSE IF *format=='-' THEN adj='l'; format++; ELSE adj='r'; FI
		 ...
		 switch (c = *format++) {
		    ...
		    case 'd':
		        s = sprintf(digits,"%d",(short)va_arg(args,int));
			break ;
		    case 'u':
		        s = sprintf(digits,"%u",(u_short)va_arg(args,int));
			break;
		    case 'o':
		        s = sprintf(digits,"%#o",(u_short)va_arg(args,int));
			break;
		    case 'q':
			s = print_signed_oct(digits,(short)va_arg(args,int));
			break;
		    case 'x':
		        s = sprintf(digits,"%x",(u_short)va_arg(args,int));
			break;
		    case 'r':
		        s = printradix(digits,
			...
			break;
		    case 'D':
		    	s = sprintf(digits,"%ld",va_arg(args,long));
			break;
		    case 'U':
		    	s = sprintf(digits,"%lu",va_arg(args,long));
			break;
		    case 'O':
			s = sprintf(digits,"%l#o",va_arg(args,long));
			break;
		    case 'Q':
			s = print_signed_oct(digits,va_arg(args,long));
			break;
		    case 'X':
		    	s = sprintf(digits,"%lx",va_arg(args,long));
			break;
		    case 'c':
		    ...
		    case 'f':
		    case 'F':
			s = sprintf(digits,"%+.16e",va_arg(args,double));
			prec= -1;
			break;
		    case 'm':
			break;
