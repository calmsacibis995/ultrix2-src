 ...
 *	003 - Added a check so that nm doesn't die with an error        *
 *	      message when the .__SYMDEF file begins with a valid       *
 *	      magic number. (Lu Anne Van de Pas, Sept 30,1985)          * 
 *									*
 *	002 - Added N_NOMAP (no DST mapping for symbol).		*
 *	      Added N_MOD2 (used by modula2 imc).			*
 *	      (vjh, June 26, 1985)					*
 *									*
 *	001 - Added support for N_NSYMS.				*
...
namelist()
{
	...
	do {
		...
		if (N_BADMAG(mag_un.mag_exp))
			continue;
		/* 003 -- if name equals .__SYMDEF the file is ok */ 
