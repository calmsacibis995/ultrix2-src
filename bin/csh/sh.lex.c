#ifndef lint
static char *sccsid = "@(#)sh.lex.c	1.2 (Ultrix) 12/9/85";

/*
    Orig ID: "@(#)sh.lex.c 4.2 2/9/83";
*/
#endif
...
/*
    ...
    There is some involved processing here, because of the
    complications of input buffering, and especially because of
    history substitution.
 ------------
 Modification History
 ~~~~~~~~~~~~~~~~~~~
01	27-Nov-85, Greg Tarsa
	Fixed the problem where alphanumeric variables that could be
	initialized with the set command could not be expanded when
	inside curly braces, foreach parenthesis, etc.  The problem
	was in the getdol() routine checking for a leading number to
	identify the special $argv[n] variable names ($1, $2, etc)
	and then assuming if the first character was not a digit, then
	the remaining characters had to be letters, not alphanumerics.
*/
/************************************************************************
 *									*
 *			Copyright (c) 1985 by				*
...
getdol()
{
	...
	*np++ = c;
	...
	default:
		/*
		 OK, its a regular variable.
		 Is it a special name for argv[n]?
		*/
...
*/
		...
		/*
		 GT01: changed letter() to alnum() below to allow
		 variables like a2z to be expanded.

		 No, is it a valid variable name?
		*/
		} else if (alnum(c))
