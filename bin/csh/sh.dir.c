#ifndef lint
static char *sccsid = "@(#)sh.dir.c	1.2	ULTRIX	10/3/86";
/* Original Id: "@(#)sh.dir.c 4.2 2/3/83" */
#endif
 ...
 *
 ...
 ~~~~~~~~~~~~~~~~~~~
01	5-Jun-86, Greg Tarsa
	Failure of getwd() was being output to defunct file descriptor
	2 instead of FSHDIAG and therefore being lost.
	...
	char *hp;
{
	...
	if (loginsh && hp)
		...
		if (cp == NULL) {
			char *tmp;
			write(FSHDIAG, path, strlen(path));
			tmp = "-- Possibly bad mode on a parent directory\n";
			write(FSHDIAG,tmp,strlen(tmp));
			exit(1);
		}
	}
