#ifndef lint
static char *sccsid = "@(#)sh.set.c	1.2 (Ultrix) 12/9/85";

/*
    Orig ID: "@(#)sh.set.c 4.1 10/9/80";
*/
#endif
...
01	27-Nov-85, Greg Tarsa
	Fixed problem where set was allowing an invalid variable
	name to be manipulated. Alphanumeric variables must now
	begin with an alphabetic (to distinguish them from argv[]
	special names).
*/
/************************************************************************
 *									*
...
#define oops(reason)	{ errstr = reason; goto setsyn; }
	...
	do {
		...
		/*
		 GT01:
		 Any variable being set with the set command
		 must start with an alphabetic.
		*/
		if (!letter(*p))
			oops("Syntax error: name needs leading alphabetic");

		for (vp = p; alnum(*p); p++)
			continue;

		if (vp == p)
			oops("Syntax error: name contains bad character");
