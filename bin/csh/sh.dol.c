#ifndef lint
static char *sccsid = "@(#)sh.dol.c	1.2 (Ultrix) 12/9/85";

/*
    Orig ID: "@(#)sh.dol.c 4.4 5/3/83"
*/
#endif
...
/*
    ...
    These routines perform variable substitution and quoting via
    ' and ". To this point these constructs have been preserved
    in the divided input words.  Here we expand variables and
    turn quoting via ' and " into QUOTE bits on characters (which
    prevent further interpretation). If the `:q' modifier was
    applied during history expansion, then some QUOTEing may have
    occurred already, so we dont "scan(,&trim)" here.

 ------------
 Modification History
 ~~~~~~~~~~~~~~~~~~~
01	27-Nov-85, Greg Tarsa
	Changed code to flag alphanumeric variable names
	starting with a digit to be invalid.  This was not actually
	a problem in this code, since any variable starting with a
	digit is assumed to be an argv[n] variable, but this module
	is now consistent with the others that had to be changed.
*/
/************************************************************************
 *									*
 *			Copyright (c) 1985 by				*
