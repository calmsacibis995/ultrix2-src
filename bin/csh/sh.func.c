#ifndef lint
static char *sccsid = "@(#)sh.func.c	1.6	ULTRIX	10/3/86";

/*
    Orig ID: "@(#)sh.func.c 4.10 83/06/11"
*/
#endif
 ...
 * C shell
 ...
 ~~~~~~~~~~~~~~~~~~~
02	11-Aug-86, Greg Tarsa
	Fixed getword to include words terminated by opening parens
	as well as whitespace.  This fixed a problem where
	control constructs that had parenthesized lists
	concatenated to them were not being recognized 
	the outer loop terminated.
	...
	The above produced "should never execute" followed by a
	"end: Not in while/foreach" error.

01	27-Nov-85, Greg Tarsa
	Fixed problem where foreach was rejecting valid variable
	names.  Now any alphanumeric with a leading alphabetic
	will be accepted.  Also expanded error messages.
*/
/************************************************************************
 *									*
 *			Copyright (c) 1985 by				*
...
/*
    ...
    Return the next word in the input.  A word in this routine
    is terminated by whitespace or an opening parenthesis.
*/
getword (wp)
register char  *wp;
{
    ...
    c = readc (1);		/* get a character */
    d = 0;

    do
	{
	/* Skip word terminators */
	while (c == ' '		/* skip whitespace */
		|| c == '\t'
		|| c == '(')	/* GT02: skip opening parens */
	    c = readc (1);
	...
	if (c == '\n')			/* EOL hit? */
	    ...
	    return (0);			/* or return empty */
	    }

	unreadc (c);			/* push good char back */

	found = 1;			/* we have text */

	do
	    {
	    c = readc (1);		/* get character */
	    ...
	    /*
	      Quotation flag processing.
	      if d is set:
		    a quote is open and d is the type of quote
		    that will close it.
	      if d is clear:
	      	    a quote is starting.  Set d to the type of quote.
	     */
	    if (any (c, "'\""))
		if (d == 0)
		    d = c;
	    ...
	    if (c < 0)			/* EOF, we're done */
		goto past;

	    if (wp)			/* put char into word */
		{
		*wp++ = c;
		}
	    len++;
	    }
	while ((d 		/* break non-quoted word . . .*/
		|| c != ' '	/* at whitesp . . . */
		   && c != '\t'
		   && c != '(') /* GT02: or at opening paren */
		&& c != '\n');	/* AND while c != EOL */
	}
    while (wp == 0);
