...
**	EDIT HISTORY:
**		15-Dec-1986: 0001:
**		Marc Teitelbaum, DEC Ultrix Engineering.
**			Detach syslog from controlling tty. Establish
**			a new process group (the pid), so that opens
**			on tty devices don't establish themselves as
**			controlling terminals.  Open tty logs per
**			message and close, otherwise vhangup will
**			dereference the descriptor on us when someone
**			logs out.
**		7-Jan-1987: 0002:
**		Marc Teitelbaum, via help from puivax!ed
**			Move opening of tty to later when we are
**			committed to the write.  Otherwise we open
**			ttys and never close them. (oops)
*/
...
# endif LOG_IPC
	...
	if (!Debug)
	{
		 ...
		 * 0001 - marc
		 *
		 * Disassociate controlling terminal and establish 
		 * new process group to prevent opens on tty devices
		 * from sticking to us.
		 * 
		 */
		d = open("/dev/tty", 2);
	...
	char	*msg;
{
	...
	for (i = 0; i < NLOGS; i++)
	{
		...
		/* 0002 - puivax!ed */
		/* if it isn't a tty and the file descriptor is NULL do nothing */
		/* if it is a tty then keep going, ttys should always be closed */
		if ( ! f->f_tty && f->f_file == NULL)
			continue;
		...
		/* 0002 - puivax!ed moved from 10 lines above */
		/* we've decided to really write this message to the file so... */
		/* ttys are opened and closed for each write 0001 */
		if (f->f_tty && f->f_file == NULL)
			f->f_file = fopen(f->f_name, "a");
		...
		if (ferror(f->f_file))
		{
			...
			fclose(f->f_file);
			f->f_file = NULL;
			f->f_tty = 0;	/* otherwise would reopen */
	...
	*/
	...
	while (fgets(cline, sizeof cline, cf) != NULL)
	{
		...
		*p = '\0';
		...
		/*
		 * tty lines have to be opened and closed for each message.
		 *
		 * 0001
		 */
		if (f->f_tty)
