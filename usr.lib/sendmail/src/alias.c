...
**	********************
**	10/01/86 - Fred L. Templin
**		Rearranged code in initaliases() to properly use the
**		dbm(3) functions for the case of initializing aliases
**		out of a database other than "/usr/lib/aliases".
*/
...
# ifdef DBM
	...
	if (!init) {
		...
		**	If not, we will assume that someone died, and
		**	it is up to us to rebuild it.
		*/

		dbminit(aliasfile);
		atcnt = 10;
		while (SafeAlias && atcnt-- >= 0 && aliaslookup("@") == NULL)
			sleep(30);
		...
		*/
		...
		(void) strcat(buf, ".pag");
		stb.st_ino = 0;
		if (atcnt < 0 || stat(buf, &stb) < 0 || stb.st_mtime < modtime)
		{
			errno = 0;
	...
	if (init)
	{
		...
		**	If running without DBM, load the symbol
		**	table. 	After loading the DBM file, add
		**	the distinquished alias "@".
		*/
