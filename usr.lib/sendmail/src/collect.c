...
#ifndef lint
static	char	*sccsid = "@(#)collect.c	1.3	(ULTRIX)	4/25/86";
#endif	lint
...
# endif NOTUNIX
	...
	*/
	...
	/* Add  ferror  to termination condition to prevent infinte
	 * loops on an error.
	 */
		 ...
		 sfgets(buf, MAXFIELD, InChannel) != NULL)
	{
		...
		if (index(buf, '\n') == NULL)
		{
			/* Avoid infinite loop if error occurs
			 * while in the loop.
...
# endif DEBUG
	...
	*/
	/* Add  ferror  to termination condition to prevent infinite
	 * loops on error conditions.
	 */
	for (; !feof(InChannel) && !ferror(InChannel);
		!feof(InChannel) && !ferror(InChannel) &&
	...
	/* Add  RealHostName  to error message to help track
	 * down problems.
	 */
	if (feof(InChannel) && OpMode == MD_SMTP)
		syserr("collect: %s unexpected close",RealHostName);

	/* Add a test for error and do syserr if found. This is part
	 * of attempted infinite loop prevention.
	 */
	if (ferror(InChannel))
		syserr("collect: %s unexpected close",RealHostName);
