#ifndef lint
static char *sccsid = "@(#)conn.c	1.13	ULTRIX	2/12/87";
#endif
 ...
 *	decvax!marc - add linebaudrate so packet driver can be smart
 *		about how long to pause between reads.
 */
...
int linebaudrate;	/* set here - used in pk driver (and fio.c) */
...
{
	if (Dcf > 0)
		ioctl(Dcf, TIOCNCAR); /* insure writes dont hang */
	(*(CU_end))(Dcf);
...
again:
	...
	if (buf[0] == '#') goto again;
	na = sscanf(buf, "%s%s%s%s%s%s", dev->D_type, dev->D_line,
	  dev->D_calldev, dev->D_class, dev->D_brand, dev->D_proto);
	ASSERT(na >= 4, "BAD DEVICE ENTRY", buf, 0);
	if (na < 6)  dev->D_proto[0] = '\0';
