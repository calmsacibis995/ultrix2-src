#ifndef lint
static char sccsid[] = "@(#) @(#)cico.c	1.6  12/16/86 ";
#endif
 ...
 *	  -f : force connection attempt regardless of previous connect status.
 *
...
*/
		...
		if (ttyn != NULL)
		{
			char msg[100];
			chmod(ttyn, 0600);
			sprintf(msg, "on %s", ttyn);
			logent(msg, "incoming call");
		}
	}
loop:
	if (!onesys) {
	...
	if (Role == MASTER) {
		...
		if (Ifn != -1 && Role == MASTER) {
			ioctl(Ofn, TIOCNCAR); /* ensure write does not hang */
		...
		/* Prefproto may get set during the call to conn() if     */
		/* the dialer routine wishes to set it.  Currently this   */
		/* is used to select the f-proto for lines directly 	  */
		/* connected to a PAD.					  */
		/* Prefproto is used in cntrl.c:fptcl() when negotiating */
		/* a common protocol.					  */
		Prefproto = DEFAULTPROTO;
		Ofn = Ifn = conn(Rmtname);
		if (Ofn < 0) {
			sprintf(msg, "call to %s, %s ", Rmtname,Stattext[-Ofn]);
			logent(msg, "FAILED");
			UB_SST(-Ofn);
...
int code;
{
	...
	if (Role == SLAVE) {
		...
		if (Role == MASTER) {
			...
			if (Ofn != -1) {
				ioctl(Ofn, TIOCNCAR); /* ensure write
						       *  does not hang 
						       */
				write(Ofn, EOTMSG, strlen(EOTMSG));
			}
