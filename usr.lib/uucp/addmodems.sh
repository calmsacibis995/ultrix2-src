#!/bin/sh5
#  @(#)addmodems.sh	1.9	ULTRIX	2/13/87
#
...
y)
	...
	;;
*)
	UULIB=/usr/lib/uucp ; ACUCAP=/etc/acucap ; FILECMD=/usr/bin/file
...
#DIALERS="$DIALERS direct"  # direct connect lines - in a future version
...
"
	...
	ventel)	SPEEDS="$VENTELSPEED" ;; *)	SPEEDS="$DEFAULTSPEED" ;;
	esac
	set ""$SPEEDS
	while :
	do
...
EOF
	...
	case "$dir" in "shared"|"out")
	    echo "g/$newname/d\nw\nq" | ed - $LDEVICES
	    echo "ACU	$newname $newname $speed $type" >> $LDEVICES
	esac
...
file: /usr/lib/uucp/modemdb.  You should now physically connect
