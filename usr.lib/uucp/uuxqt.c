#ifndef lint
static char sccsid[] = "@(#)uuxqt.c	1.5 (decvax!larry) 6/12/85";
#endif
 ...
 *		     - D.files were not being removed - fixed 12/12/85
 *****************/
...
char *argv[];
{
	...
	for (i=0; i<NCMDS-1 && cfgets(cmd, sizeof(cmd), fp) != NULL; i++) {
		...
		if (strncmp(argp[0], "PATH=", 5) == 0) {
			/* kludge to change path in L.cmd file */
			strcpy(PATH, argp[0]);
			i--; /* kludge */
			continue;
		}
...
doprocess:  /* process work in the spool directory */
	  ...
	  while (gtxfile(xfile) > 0) {
		...
		if ((xfp = fopen(subfile(xfile), "r")) == NULL) {
			ASSERT_NOFAIL(xfp != NULL,"uuxqt collision - no damage",
				xfile, 0);
			continue;
		}
		if (flock(fileno(xfp), LOCK_EX|LOCK_NB) < 0) {
/*
			ASSERT_NOFAIL(0,"uuxqt collision check - no damage",
				xfile, 0);
*/
			fclose(xfp);
			continue;
...
*/
		...
		sprintf(lbuf, "%s %s XQT", user, xfile);
		logent(buf, lbuf);
		DEBUG(4, "cmd %s\n", buf);
...
/* watcgl.11, dmmartindale, signal and exit values were reversed */
		 ...
		 * 2.  ok to notify user  && exit status was non zero &&
				ok to send non zero exit status
	...
	rmfiles:
		...
		while (fgets(buf, BUFSIZ, xfp) != NULL) {
			if (buf[0] != X_RQDFILE) 
				continue;
			sscanf(&buf[1], "%s", file);
			ret=unlink(subfile(file));
		}
		fclose(xfp);
		ret=unlink(subfile(xfile));
		ASSERT_NOFAIL(ret != -1,"CAN NOT UNLINK", subfile(xfile),errno);
		unlink(subfile(ferr));
	  } /* end doprocess loop */
	} /* end nextsys loop */
...
char *file;
{
	static int reopened;
	static DIR *dirp;
	static char xlock[MAXFULLNAME] = { '\0' };
	char pre[3];
...
retry:
	...
	while (gnamef(dirp, file) != 0) {
		 ...
		 * uuxqts.
		 */
		sprintf(xlock,"%s/LCK.%s",SPOOL,file);
...
{
	FILE *fp;
	char buf[BUFSIZ], file[MAXFULLNAME], tfile[MAXFULLNAME];
...
{
	FILE *fp;
	char buf[BUFSIZ], ffile[MAXFULLNAME], tfile[MAXFULLNAME];
	char tfull[MAXFULLNAME];
	int ret;
