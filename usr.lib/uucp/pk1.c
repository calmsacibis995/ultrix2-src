#ifndef lint
static char sccsid[] = "@(#)pk1.c	1.7 (Berkeley-4.3) 2/12/87";
#endif
...
extern int errno;
/** extern int Retries; removed - marc */
...
int ifn, ofn;
{
	...
	if (i == 0) {
		PKDEBUG(1, "pkopen: can't malloc i = 0\n", CNULL);
		return NULL;
	}
	pk->p_rwindow = i;
...
register struct pack *pk;
{
	...
	for (tries = 0, noise = 0; tries < GETRIES; ) {
		...
		if (pkcget(ifn, p, 1) == SUCCESS) {
			...
			PKDEBUG(4, "Noisy line - set up RXMIT\n", CNULL);
			noise = 0;
		}
	...
	sum |= (unsigned) *p << 8;
	h->sum = sum;
	PKDEBUG(7, "rec h->cntl 0%o\n", h->cntl&0xff);
	k = h->ksize;
	if (hdchk != h->ccntl) {
		/* bad header */
		PKDEBUG(7, "bad header 0%o,", hdchk&0xff);
		PKDEBUG(7, "h->ccntl 0%o\n", h->ccntl&0xff);
		return;
	}
	if (k == 9) {
		if (((h->sum + h->cntl) & 0xffff) == CHECK) {
			...
			/*  bad header */
			pk->p_state |= BADFRAME;
			PKDEBUG(7, "bad header (k==9) 0%o\n", h->cntl&0xff);
		}
		return;
	}
	if (k && pksizes[k] == pk->p_rsize) {
		pk->p_rpr = h->cntl & MOD8;
		PKDEBUG(7,"end pksack 0%o\n", pk->p_rpr);
		pksack(pk);
		bp = pk->p_ipool;
		if (bp == NULL) {
			PKDEBUG(7, "bp NULL %s\n", "");
	...
	while (n > 0) {
		...
		if (linebaudrate > 0) {
			...
			if (r > 20000) {
				...
				PKDEBUG(11, "PKCGET stall for %d", tv.tv_sec);
				PKDEBUG(11, ".%06d sec\n", tv.tv_usec);
