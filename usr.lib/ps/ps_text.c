#ifndef lint
static char *sccsid = "@(#)ps_text.c	1.3	ULTRIX	10/3/86";
#endif lint
/************************************************************************
 *									*
 *			Copyright (c) 1986 by				*
...
/************************************************************************/
/*									*/
/*	ps_text.c - text file print filter for PostScript Printer	*/
/*									*/
...
/*				                                        */
/*				                                        */
/*	Modify for Postscript Magic Number				*/
/*		--Robert Chesler, 8/5/86				*/
/*				                                        */
/*	Modify for PostScript End of File				*/
/*		--Robert Chesler, 8/13/86				*/
/*				                                        */
/************************************************************************/
...
#define	FONT		"Courier"	/* default font */
					
#define PS_EOF 		'\004'		/* PostScript End of File */
#define BACKSPACE	'\010'		/* Back Space overstrike character */
...
char	usage[] = "Valid ps_text options:\n\t-O[offset]\n\t-r[rotation]\n\t-s[fontsize]\n\t-T[tabsize]\n\t-p[pitch]\n\t-f[font]\n\t-h[horizon space]\n\t-m\n\t-2\n\t-l[page length]\n\t-t[input tray]\n\t-o[output tray]\n";
...
#else
#define spgetc(f) ((*sp) ? (*sp++) : (getc(f)))
#endif 

/*	PSMN modification end */
...
/*
***	If the file is PostScript, then copy it to output
 */
	if (is_ps(istr)==PS)
	{
		while ((ch = spgetc(istr)) != EOF)
			putc(ch,stdout);
		putchar(PS_EOF);
		return ;
	}
