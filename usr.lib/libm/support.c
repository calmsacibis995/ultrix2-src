#ifndef lint
static char	*sccsid = "@(#)support.c	1.1	(ULTRIX)	5/1/86";
#endif lint
...
*
*		David Metsky,	17-Jan-86
*
* 001	Replaced old version with BSD 4.3 version as part of upgrade
*
*	Based on:	support.c 	1.1 (Berkeley)	5/23/85
*
*************************************************************************/
...
#include <math.h>
    ...
    static short prep1=54, gap=4, bias=1025           ;
...
#ifdef VAX
        if( (k= *px & mexp ) != ~msign ) {
            if( N<-260) return(0.0); else if(N>260) return(HUGE);
#else   /* IEEE */
        if( (k= *px & mexp ) != mexp ) {
            if( N<-2100) return(0.0); else if(N>2100) return(HUGE);
            if( k == 0 ) {
            ...
            if((k = (k>>gap)+ N) > 0 )
                ...
                else {errno = EDOM; x=HUGE;}       /* overflow */
            else
                if( k > -prep1 ) 
                                        /* gradual underflow */
...
double x;
{
    ...
    /* sqrt(negative) is invalid */
        if(x<zero) {errno = EDOM; return(zero);}
