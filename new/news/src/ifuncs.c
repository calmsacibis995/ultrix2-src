...
#ifndef lint
static char *Sccsid = "@(#)ifuncs.c	1.2	12/10/84";
#endif

/*
 * static char *SccsId = "@(#)ifuncs.c	1.5 5/17/84 (from 2.21 3/31/83 news-2.10.1)";
 */

/* ------------------------------------------------------------------------
 * Modification History: /usr/src/local/news-2.10.1/src/ifuncs.c
 *
 * 17 May 84 --jmcg
 *	Max open files exceeded because `transmit' can return without
 *	closing the stream that was opened to pass as its argument.
 *	Repeatedly opening the file seems inefficient, but for a quick
 *	surgical fix, I simple matched each `return' with an `fclose'.
 *
 * 16 May 84 -- jmcg
 *	Added Q flag.  Works like F flag, but article id is included
 *	to facilitate a batched version of ihave/sendme.  The remainder
 *	of this mechanism is unimplemented.
 *
 * 16 May 84 --jmcg
 *	If a subscriber site uses batching (F) or copy (U) mode, the
 *	hh structure was used but not initialized.  Re-reading the article
 *	can be bypassed in these cases by passing the article identification
 *	as an additional argument to `transmit'.
 *
 *  7 Mar 84 --jmcg
 *	Moved lock and unlock calls.
 *	Reduces lock granularity to allow simultaneous rnews execution
 *	streams.  Without this, they simply lock each other out most of
 *	the time, with occasional timeouts and confusion from removed
 *	lock files.  We've been running with this change for a number
 *	of months now, so it appears to be stable.
 *
 * ------------------------------------------------------------------------
 */
	...
	*nptr = '\0';
	...
	while (s_read(&srec)) {
		...
		if (ngmatch(h.nbuf, srec.s_nbuf)) {
			transmit(&srec, xfopen(ARTICLE, "r"), 1, h.ident);
		}
	contin:;
	}
	s_close();
...
#define PROC 0004
/*VARARGS3*/
transmit(sp, ifp, maynotify, identp)
...
/* Q:	qappfile: append article id and name to file */
	int qappfile = (index(sp->s_flags, 'Q') != NULL);
...
/* U:	useexist: use the -c option to uux to use the existing copy */
	...
	if (!appfile && !qappfile && !useexist) {
		...
		*/
		...
		/*
		 * for appfile, qappfile, and useexist, this is the only
		 * part of the hh structure that is required.
		 */
		strcpy( hh.ident, identp);
		}
