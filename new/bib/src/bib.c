#ifndef lint
static	char	*sccsid = "@(#)bib.c	1.2	(ULTRIX)	4/23/85";
#endif lint
 ...
 * 0001	Change tmpfile to txtfile so that it doesn't clash with 	*
...
/* global variables */
   ...
   char txtfile[] = TMPTEXTFILE ; /* output of pass 1                    */
...
{  int rcomp();
   ...
   /* open temporaries, reffile will contain references collected in
      pass 1, and txtfile will contain text.
   */
   mktemp(reffile);
   rfd = fopen(reffile,"w+");
   if (rfd == NULL)
