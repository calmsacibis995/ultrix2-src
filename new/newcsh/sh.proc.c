static	char *sccsid = "@(#)sh.proc.c	1.1 (tarsa) 10/14/84";
/*
    sh.proc.c
	
    C Shell functions that manage processes,
    handling hanging, termination.

------------
 Modification History
 ~~~~~~~~~~~~~~~~~~~
 02	Greg Tarsa, 2-Mar-84
	- Changed so as not to use stdio.

 01	Greg Tarsa, 16-Jan-84
	- Added optional bell to process completion messages.
...
found:
	...
	jobflags = 0;	/* the set of flags set in the job set */
	fp = pp;	/* init the "floating" pointer */

	/*
	 Get the status of the processes in the job list (pipe).
	 If "time" is set, then set the flag to report time
	 if any process exceeds the threshold time value.
	*/
	do {
		if ((fp->p_flags & (PPTIME|PRUNNING|PSTOPPED)) == 0 &&
		    !child && adrof("time") &&
		    (fp->p_utime + fp->p_stime) / HZ >= atoi(value("time")))
			fp->p_flags |= PTIME;
	...
	if ((jobflags & (PRUNNING|PREPORTED)) == 0) {
		...
		/* mark stopped jobs as reported already */
		do {
			if (fp->p_flags&PSTOPPED)
				fp->p_flags |= PREPORTED;
		} while((fp = fp->p_friends) != pp);

		/* move fp around to job leader */
