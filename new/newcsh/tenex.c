...
*/
#define CMD_BUILTIN	-2	/* get from builtin command table */
#define CMD_ALIAS	-1	/* get from alias list */
...
int dirctr;
{
	...
	case VAR_WD:		/* get a variable entry */
	    ...
	    context->curvar = context->curvar->link;
...
{
    register char *wp;
    register struct wordent *ep = end;
...
*/
char *get_dir_from_path (path, dir, dirctr, dirflag)
...
int (*routine) ();
{
    ...
    int  dirctr;	/* -2 -1 (for BUILTINS), 0 1 2 ... (for dirlist pos)*/
