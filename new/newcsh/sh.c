 ...
 *
 ...
 04	8-Mar-84, Greg Tarsa, Digital Equipment Corp.
	Fixed an erroneous data type.

 03	October 1983, Ken Greer, Ind. Consultant, Palo Alto, CA.
	Added Filename recognition.

 02	April 1980, Jim Kulp, IIASA, Laxenburg, Austria.
	
 01	October 1978, Bill Joy, UC Berkeley, California, USA.
	Wrote original code.
*/
...
mailchk()
{
	...
	for (; *vp; vp++) {
		if (stat(*vp, &stb) < 0)
			continue;
		new = stb.st_mtime > time0.tv_sec; /* correc: .tv_sec */
	...
	int i;
{
	...
	untty();
#ifdef PROF
	_cleanup();	/* force out the last bufferful */
	/* IEH3exit(i); */
#else
	_exit(i);
#endif
