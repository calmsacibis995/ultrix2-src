#ifndef lint
static char *sccsid = "@(#)yppoll.c	1.1	ULTRIX	10/3/86";
#endif lint
...
/*
 * This is a user command which asks a particular ypserv which version
 * of a map it is using.  Usage is:
 * 
 * yppoll [-h <host>] [-d <domainname>] mapname
 * 
 * where host may be either a name or an internet address of form
 * ww.xx.yy.zz
 * 
 * If the host is ommitted, the local host will be used.
 * If host is specified as an internet address, no yp services need to
