#ifndef lint
static char *sccsid = "@(#)ypxfr.c	1.2	ULTRIX	10/3/86";
#endif lint
 ...
 * enumeration functions.  The map is copied to a temp name, then the
 * real map is removed and the temp map is moved to the real name.
 * ypxfr then  sends a "YPPROC_CLEAR" message to the local server to
 * insure that he will not hold a removed map open, so serving an
 * obsolete version.  
 * 
 * ypxfr [-h <host>] [-d <domainname>] [-f] [-c] [-C tid prot ipadd port] map
 * 
 * where host may be either a name or an internet address of
 * form  ww.xx.yy.zz
 * 
 * If the host is ommitted, ypxfr will attempt to discover the master
 * by using normal yp services.  If it can't get the record, it will use
 ...
 * 
 * If the domain is not specified, the default domain of the local
 * machine is used.
 * 
 * If the -f flag is used, the transfer will be done even if the
 * master's copy is not newer than the local copy.
 *
 * The -c flag suppresses the YPPROC_CLEAR request to the local ypserv.
 * It may be used if ypserv isn't currently running to suppress the
 * error message.
 * 
 ...
 * running on the node with IP address "ipadd", and is listening (UDP)
 * on "port".  
 *  
 */
#include <dbm.h>
		...
		if ( (*argv)[0] == '-') {
			...
			case 'C': {
				...
				if (argc > 5) {
					...
					argc -= 4;
#ifdef DEBUG
logprintf("callee  ipadd = %s\n",ipadd);
logprintf("port = %s --or-- %d\n",port,htons(atoi(port)));
logprintf("proto = %s\n",proto);
logprintf("tid = %s\n",tid);
#endif DEBUG
				} else {
...
/* ORIGINALSUN code  had   htons(atoi(proto)) in the following call.
 * It fails..
 */
	if ((domb.dom_client = clntudp_create(&(domb.dom_server_addr),
		...
		clnt_perror(domb.dom_client, "ypxfr:  clntudp_create failure");
		*status = YPPUSH_RPC;
		return;
	}	
	...
	if((enum clnt_stat) clnt_call(domb.dom_client,
	    ...
	    udp_timeout) != RPC_SUCCESS) {
		clnt_perror(domb.dom_client, "ypxfr:  clnt_call back failure");
		*status = YPPUSH_RPC;
		return;
	} 
}
