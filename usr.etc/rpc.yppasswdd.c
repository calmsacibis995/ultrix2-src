#ifndef lint
static char *sccsid = "@(#)rpc.yppasswdd.c	1.2	ULTRIX	10/3/86";
#endif lint
...
/*	Revsions
 *	--------
 * Jul 30,1986	rjg/ Modified "rename" of temporary passwd file
 *		so that rpc.yppasswdd would work if the source
 *		passwd file existed on another file system.
 *		ie. if  /etc/yp  is a symbolic link to  /usr/etc/yp
 *		the rename would fail. Can't rename across devices.
 *		The Sun makefile at installation time creates a
 *		link  /etc/yp  to  /usr/etc/yp  to save space on
	...
	SVCXPRT *transp;
{
...
#ifdef ORIGINALSUN
	/* Original version of Sun code that fails when  /etc/yp
	 * is a symbolic link to  /usr/etc/yp
	 */
	...
	/* Set up a command to copy the temp file to the
