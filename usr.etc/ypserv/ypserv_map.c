#ifndef lint
static char *sccsid = "@(#)ypserv_map.c	1.1	ULTRIX	10/3/86";
#endif lint
...
/*
 * This performs an existence check on the dbm data base files
 * <name>.pag  and <name>.dir.
 * pname is a ptr to the filename.  This should be an absolute path.
 * Returns TRUE if the map exists and is accessable; else FALSE.
 *
 * Note:  The file name should be a "base" form, without a file
 * "extension" of  .dir or .pag appended.
 * See ypmkfilename for a function which will generate the name
 * correctly.  Errors in the stat call will be reported at this level,
 * however, the non-existence of a file is not considered an error,
