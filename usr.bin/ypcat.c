...
#ifndef lint
static char *sccsid = "@(#)ypcat.c	1.2	ULTRIX	10/3/86";
#endif lint
...
/*
 * This is a user command which dumps each entry in a yp data base.
 * It gets the info using the normal ypclnt package.
 * The user doesn't get to choose which server gives him the input.
 * Usage is:
 * ypcat [-k] [-d domain] [-t] map
 * ypcat -x
 * where the -k switch will dump keys followed by a single blank space
 * before the value, and the -d switch can be used to specify a domain
 * other than the default domain.
 * 
 * Normally, passwd gets converted to passwd.byname, and similarly for
 * the other standard files.  -t inhibits this translation.
 * 
