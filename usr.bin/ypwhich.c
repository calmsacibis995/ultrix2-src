#ifndef lint
static char *sccsid = "@(#)ypwhich.c	1.2	ULTRIX	1/15/87";
#endif lint
 ...
 * default domain.  -m tells the master of that map.
 * mname may be either a  mapname, or a nickname which will be
 * translated into a mapname according to the translation table at
 * transtable.  The  -t switch inhibits this translation.
 * If the -m option is used, ypwhich will act like a vanilla
 * yp client, and will not attempt to choose a particular yp server.
 * On the other hand, if no -m switch is used, ypwhich will talk
 * directly to the yp bind process on the named host, or to the local
 * ypbind process if no host name is specified.  The -x switch can be
 * used to dump the nickname translation table.
 *  
 */
