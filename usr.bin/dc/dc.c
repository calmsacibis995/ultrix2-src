#ifndef lint
static	char	*sccsid = "@(#)dc.c	1.2	(ULTRIX)	11/25/85";
#endif lint
 ...
 *
 *  001  T.N. Cherng	Nov. 19, 1985
 *	The bc command is a preprocessor for dc this command.
 *	1. The error message from dc is stdout instead of stderr, 
 *	      when running bc won't generate the error message (fixed)
 *	2. Once the operation is over stack size, a core dump will occur
 *		instead of exit (fixed).
 *
 *	based on: dc.c	4.2	(Berkeley)	3/30/83";
 *
 ************************************************************************/
...
casediv:
		...
		case 'q':
			...
			continue;
		case 'f':
			if(stkptr == &stack[0])fprintf(stderr,"empty stack\n");
			else {
...
execute:
			...
			if((readptr != &readstk[0]) && (*readptr != 0)){
				if((*readptr)->rd == (*readptr)->wt)
					...
					/* the last array should be RDSKSZ-1*/
						errorrt("nesting depth\n");
					}
				}
			}
			else readptr++;
...
{
	char *p;
	fprintf(stderr,"%s %o rd %o wt %o beg %o last %o\n",s1,hptr,hptr->rd,hptr->wt,hptr->beg,hptr->last);
	p = hptr->beg;
	while(p < hptr->wt)fprintf(stderr,"%d ",*p++);
	fprintf(stderr, "\n");
}
seekc(hptr,n)
struct blk *hptr;
...
char *s;
{
	fprintf(stderr,"out of space: %s\n",s);
	fprintf(stderr,"all %ld rel %ld headmor %ld\n",all,rel,headmor);
