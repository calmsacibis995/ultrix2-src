 ...
 */
#ifndef lint
static char *sccsid = "@(#)iostat.c	1.3	ULTRIX	12/16/86";
#endif lint
...
char *argv[];
{
	...
	read_names();
	lseek(mf, (long)nl[X_ACTIVECPU].n_value, 0);
	read(mf, &activecpu, sizeof (activecpu));
	if(argc > 2)
		iter = atoi(argv[2]);
loop:
	if (--tohdr == 0) {
	...
	lseek(mf, (long)nl[X_CPUDATA].n_value, 0);
	read(mf, s.cpudata, activecpu * sizeof (struct cpudata));
	for (i = 0; i < DK_NDRIVE; i++) {
#define X(fld)	t = s.fld[i]; s.fld[i] -= s1.fld[i]; s1.fld[i] = t
	...
	if (activecpu > 1) {
		for (cpindex = 0; cpindex < activecpu; cpindex++) {
			printf(" ");
			stat1(s.cpudata[cpindex].c_cptime);
		}
	}
	printf("\n");
contin:
