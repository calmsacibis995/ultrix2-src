#ifndef lint
static char *sccsid = "@(#)ranlib.c	1.4	ULTRIX	12/16/86";
#endif
...
char **argv;
{
	...
	while(argc--) {
		...
		do {
			...
			if (N_BADMAG(exp))
				continue;
			if (!strncmp(tempnm, archdr.ar_name,
			...
			if (ssiz < sizeof ssiz){
				/* sanity check */
				fprintf(stderr,
				"ranlib: %s(%s): mangled string table\n",
				*argv, archdr.ar_name);
				exit(1);
			}
...
FILE *af;
{
	...
	for (cp=archdr.ar_name;
			cp < & archdr.ar_name[sizeof(archdr.ar_name)]; cp++)
		if (*cp == ' ')
			*cp = '\0';
	...
	struct nlist *s;
{
	...
	for (cp = s->n_un.n_name; tstrtab[tssiz++] = *cp++;)
		if (tssiz > strtabsz) {
			strtabsz *= 2;
			tstrtab = (char *) realloc( tstrtab,
						strtabsz*sizeof *tstrtab);
			if (tstrtab == 0) {
				fprintf(stderr,
...
fixsize()
{
	...
	offdelta = sizeof(archdr) + sizeof (tnum) +
			tnum * sizeof(struct ranlib) + sizeof (tssiz) + tssiz;
	off = SARMAG;
	nextel(fi);
