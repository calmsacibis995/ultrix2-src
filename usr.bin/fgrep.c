#ifndef lint
static char *sccsid = "@(#)fgrep.c	1.3	(ULTRIX)	9/30/85";
/* Original ID: "@(#)fgrep.c	4.2 (Berkeley) 10/20/82" */
#endif
/*
 ...
 ~~~~~~~~~~~~~~~~~~~
 01	Greg Tarsa, 28-Dec-84
	Added copyright. Made a fix to -i option that makes it
...
char *file;
{
	...
	for (;;) {
		if (--ccount <= 0) {
			...
			blkno += ccount;
			lastread = ccount;	/* save last number read */
		}
		nstate:
			if (ccomp(c->inp, *p)) {
				c = c->nst;
		...
		if (c->out) {
			while (*p++ != '\n') {
				if (--ccount <= 0) {
					...
					if (p > &buf[BUFSIZ]) {
						if ((ccount =
						read(f, p, &buf[2*BUFSIZ] - p))
							<= 0) break;
					}
					else if ((ccount = read(f, p, BUFSIZ)) <= 0) break;
					blkno += ccount;
					lastread = ccount; /* save number */
				}
			}
	...
	succeed:	nsucc = 1;
			if (cflag) tln++;
				...
				if (bflag) {
					printf("%ld:",( blkno -
						(nlp > &buf[BUFSIZ]  ?
						&buf[BUFSIZ+lastread] - nlp :
						&buf[lastread] - nlp )) / 512);
				}
