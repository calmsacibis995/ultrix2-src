...
#ifndef lint
static	char	*sccsid = "@(#)prs.c	1.2	(ULTRIX)	12/4/84";
#endif lint
...
register char *p;
{
	if (libPW$index(p,":C:") != -1)	/* check for Comment keyword */
		HAD_CM = 1;
	if (libPW$index(p,":MR:") != -1)/* check for MR keyword */
		HAD_MR = 1;
	if (libPW$index(p,":UN:") != -1)/* check for User name keyword */
		HAD_UN = 1;
	if (libPW$index(p,":FD:") != -1)/* check for descriptive text kyword */
		HAD_FD = 1;
	if (libPW$index(p,":BD:") != -1)/* check for body keyword */
		HAD_BD = 1;
}
