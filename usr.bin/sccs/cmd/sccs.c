...
#ifndef lint
static char *sccsid = "@(#)sccs.c	1.9	ULTRIX	12/4/86";
#endif lint
 ...
 *									*
 ************************************************************************/
 /*	sccs.c		from 3BSD or 4.0BSD via circuitous routes. */
  ...
  *
  * 8Jul83 -- jmcg
  *	New directory paths: /usr/sccs
  *
  * 1Nov83 -- rjl
  *	New directory paths: /usr/local
  *
  * 27Nov84 -- depp
  *	1. Moved path macros to new header file: ../hdr/paths.h
  *	2. Fixed "g" file deletion bug in "unedit" routine.  
  *	   The "g" file was not being deleted due to ownership conflict.
  *	   To fix, the UID is set to the "real" UID temporarily while
  *	   the file deletion takes place, then reset to the "effective"
  *	   UID.
  *	3. Fixed same type of bug in "command" routine, CLEAN case.
  *	   UID was not being set to "real".
  *	4. Provided limited chghist, it will now permit a comment to 
  *	   be inserted into a delta using "cdc" command.
  *
  * 06Dec84 -- depp
  *	Modified the setting of the "real" UID in unedit, so that it doesn't 
  *	have to be temporary (see #2 of modification 27Nov84), as the system 
  *	will not permit the UID to be reset to the effective UID.
  *
  * 14Dec84 -- depp
  *	clean up of old code that was eliminate do to modification (06Dec84).
  *
  * 24 Oct 85 -- depp
  *	Insured that the command "prt" call the utility "prs" (not "prt"
  *  	which doesn't exist).
  *
  * 17 Jul 86 -- metsky
  *	Made it impossible to do a fix to a version 1.1 file.  This was
  *	creating files that couldn't be delta'ed.  Created a new error
  *	message, (sc24) "cannot use fix command on version 1.1".
  *
  * 04 Dec 86 -- metsky
  *     Fixed previous 'fix'.  Now the fix cmmand works correctly.
  *
  ******************************************************************/
...
{
	"admin",	PROG,	0,			PROGPATH(admin),
	"chghist",	CMACRO,	0,			"cdc:r",
	...
	"cdc",		PROG,	0,			PROGPATH(cdc),
	"rmdel",	PROG,	0,			PROGPATH(rmdel),
...
# ifndef SCCSDIR
	...
	if (p != NULL && p[0] != '\0')
	{
		...
		else
		{
			...
			if (pw == NULL)
			{
				usrerr("user %s does not exist (sc1)", p);
				exit(EX_USAGE);
			}
			...
			if (access(buf, 0) < 0)
			{
				...
				if (access(buf, 0) < 0)
				{
					usrerr("project %s has no source! (sc2)"
									, p);
					exit(EX_USAGE);
				}
			}
...
# endif
	...
	*/
	...
	if (cmd == NULL)
	{
		usrerr("Unknown command \"%s\" (sc4)", *ap);
		return (EX_USAGE);
	}
	...
	*/
	...
	switch (cmd->sccsoper)
	{
	  ...
	  case FIX:		/* fix a delta */
		...
		{
			usrerr("-r flag needed for fix command (sc5)");
			rval = EX_USAGE;
			break;
		}

		if (strcmp(ap[1], "-r1.1") == 0) /* metsky 7/17/86 */
						 /* metsky 12/4/86 */
		{
			usrerr("cannot use fix command on version 1.1 (sc24)");
			rval = EX_USAGE;
			break;
		}
	  ...
	  case CLEAN:
		if (bitset(REALUSER,cmd->sccsflags))
			setuid(getuid());
	  ...
	  case UNEDIT:
		...
		*np = NULL;
		...
		/* 
		 * remove g-files here rather than in "unedit", so that
		 * the UID can stay at real user -- depp (06Dec84)
		 */
		setuid(getuid());
		for (argv = np = &ap[1]; *argv != NULL; argv++)
		{
...
# endif
		...
		syserr("cannot exec %s (sc6)", cmd->sccspath);
		exit(EX_OSERR);
	  ...
	  default:
		syserr("oper %d (sc7)", cmd->sccsoper);
		exit(EX_SOFTWARE);
	}
# ifdef DEBUG
	if (Debug)
...
# endif
	...
	{
		syserr("callprog: setting stdout w/o forking (sc23)");
		exit(EX_SOFTWARE);
	}
	...
	execv(progpath, argv);
	syserr("cannot execute %s (sc6)", progpath);
	exit(EX_UNAVAILABLE);
	/*NOTREACHED*/
}

	...
	char *fn;
{
	...
	if (q[1] != 's' || q[2] != '.')
	{
		usrerr("bad file name \"%s\" (sc13)", fn);
		return (FALSE);
	}
	...
	*++q = 'p';
	...
	if (pfp == NULL)
	{
		printf("SCCS: %12s not being edited (sc14)\n", fn);
		return (FALSE);
	}
	...
	if (tfp == NULL)
	{
		usrerr("cannot create \"%s\" (sc15)", tfn);
		exit(EX_OSERR);
	}
	...
	*/
	...
	if (others)
	{
		...
		{
			syserr("cannot reopen \"%s\" (sc16)", tfn);
			exit(EX_OSERR);
		}
		if (freopen(pfn, "w", pfp) == NULL)
		{
			usrerr("cannot create \"%s\" (sc15)", pfn);
			return (FALSE);
		}
	...
	/*
	 * if g-file present,
	 * actually remove the g-file, by first setting the UID to the real
	 * user (the file's owner), unlinking the file, and finally resetting
	 * the UID to the effective user (sccs).
	 */
	if (delete)
	{
		return (TRUE);
	...
	{
		printf("SCCS: %12s not being edited by you (sc19)\n", fn);
		return (FALSE);
	}
}

/*
**  RMGFILE -- This routine will unlink a g-file.  It assumes that the UID
**	       has been set to the real user.
**
**	Parameters:
**		fn -- pointer to the g-file name
**
**	Returns:
**		0 -- g-file unlinked
**		1 -- g-file could not be unlinked
**
**	Side Effects:
**		none
*/
	...
	char *gfile;
{
	...
	if (pipe(pipev) < 0)
	{
		syserr("dodiff: pipe failed (sc20)");
		exit(EX_OSERR);
	}
	if ((pid = fork()) < 0)
	{
		syserr("dodiff: fork failed (sc8)");
		exit(EX_OSERR);
	}
	else if (pid > 0)
	{
	...
	else
	{
		...
		{
			syserr("dodiff: magic failed (sc21)");
			exit(EX_OSERR);
		}
...
gstrbotch(str1, str2)
	char	*str1, *str2;
{
	usrerr("Filename(s) too long: %s %s (sc22)", str1, str2);
}
