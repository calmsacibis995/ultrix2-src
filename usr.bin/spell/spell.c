#ifndef lint
static	char	*sccsid = "@(#)spell.c	1.5	(ULTRIX)	11/25/85";
#endif lint
 ...
 *
 * 001	T. N. Cherng, 7-Oct-1985
 *	Fix so that very long words (i.e. over WORDSIZE characters) will
 *	not cause spell to dump core.
 *
 *	Based on:  spell.c	4.1 12/18/82
 *
 ************************************************************************/
...
char **argv;
{
	...
	for(;; fprintf(file,"%s%s\n",affix,original)) {
		...
		for(ep=word; (*ep=j=getchar()) != '\n' ; ep++)
			if(j == EOF)
				exit(0);
			else if (ep > &word[WORDSIZE-1]) {
				fputs("spell : exceeded maximum word size\n",
				      stderr);
				exit(1);
				}
