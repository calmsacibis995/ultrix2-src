#ifndef lint
static char *sccsid = "@(#)yp_all.c	1.2	ULTRIX	10/3/86";
#endif lint
...
/*
 * This does the "glommed enumeration" stuff.
 * callback->foreach is the name of a function
 * which gets called per decoded key-value pair:
 * 
 * (*callback->foreach)(status, key, keylen, val, vallen, callback->data);
 *
 * If the server we get back from _yp_dobind speaks the old protocol,
 * this returns YPERR_VERS, and does not attempt to emulate the new
 * functionality by using the old protocol.
 */
int
