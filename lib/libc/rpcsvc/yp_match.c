#ifndef lint
static char *sccsid = "@(#)yp_match.c	1.4	ULTRIX	10/3/86";
#endif lint
 ...
 *   Added a call to "yp_unbind" in the function "yp_match" to release the
 *   domain binding created by "_yp_dobind"
 */
...
/*
 * Requests the yp server associated with a given domain to attempt
 * to match the passed key datum in the named map, and to return the
 * associated value  datum.
 * This part does parameter checking, and implements the "infinite"
	...
	int  *vallen;		/* returns bytes in val */
{
	...
	/*
	 * Don't return without releasing this binding!!
	 */
	yp_unbind(domain);
	return(reason);
