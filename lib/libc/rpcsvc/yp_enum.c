#ifndef lint
static char *sccsid = "@(#)yp_enum.c	1.5	ULTRIX	10/3/86";
#endif lint
 ...
 *   Added a call to "yp_unbind" in the functions "yp_first" and "yp_next
 *   to release the domain binding created by "_yp_dobind". This may hamper
 *   performace of a yp_next loop which enumerates a big map, but it should
 *   never be left to the caller to close the domain binding!
 *
 *	Fred L. Templin (templin@decvax) Sept. 17, 1986
 *   Added parameter checking in yp_next to ensure that "inkey" and "inkeylen"
 *   are non-null.
 */
	...
	int  *vallen;		/* return: bytes in val */
{
	...
	/*
	 * Free this domain binding. This may hamper performance in a long
	 * "yp_next loop", but it's better to re-bind every time than to
	 * leave the user with a messed up descriptor table!
	 */
	yp_unbind(domain);
	return(reason);
}
