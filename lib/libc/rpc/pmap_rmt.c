#ifndef lint
static char *sccsid = "@(#)pmap_rmt.c	1.3	ULTRIX	10/3/86";
#endif lint
	...
	char *buf;  /* why allocxate more when we can use existing... */
{
	...
	ifc.ifc_len = MAX_BROADCAST_SIZE;
        ...
        for (i = 0, n = ifc.ifc_len/sizeof (struct ifreq); n > 0; n--, ifr++) {
			...
			/* Get broadcast socket structure/address.
		 	 */
			if (ioctl(sock, SIOCGIFBRDADDR, (char *)&ifreq) < 0) {
				perror("broadcast: ioctl (get broadaddr)");
				continue;
			}
	...
	resultproc_t	eachresult;	/* call with each result obtained */
{
	...
	if (setsockopt(sock, SOL_SOCKET, SO_BROADCAST, &on, sizeof(on)) !=0) {
		fprintf (stderr, "clnt_broadcast: setsockopt failed...\n");
		goto done_broad;
	}
	mask = (1 << sock);
	...
	xdr_destroy(xdrs);
	/*
	 * Basic loop: broadcast a packet and wait a while for 
	 * response(s). The response timeout grows larger per iteration.
	 */
...
#ifdef DEBUG
fprintf(stderr,"baddr.sin_family = 0x%x\n",baddr.sin_family);
fprintf(stderr,"baddr.sin_port   = 0x%x\n",baddr.sin_port);
fprintf(stderr,"baddr.sin_addr   = 0x%x\n",baddr.sin_addr);
fprintf(stderr,"i = %d, nets = %d\n",i,nets);
#endif DEBUG
