#ifndef lint
static	char	*sccsid = "@(#)ctime.c	1.5	(ULTRIX)	2/12/87";
#endif lint
 ...
 *									*
 * 003	Paul R. Shaughnessy, 07-Aug-1986				*
 *	Changed usdaytab to use new daylight savings times.		*
 *	Changed European tables to default, if no daylight		*
 *	savings time is in the table.					*
 *									*
 *	David L Ballenger, 30-Oct-1985					*
 * 002	Modify localtime so that it only calls gettimeofday() to get	*
 *	the timezone on the first call.					*
 *									*
 *	Also, add code to asctime() to fix problems with dates beyond	*
 *	Dec 31, 1999.  Fixes SPR ICA-585.				*
 *									*
 *	David L. Ballenger						*
 * 0001 Fix ctime(), localtime(), and gmtime() so that they again	*
 *	handle negative GMT times such as might be obtained with local	*
 *	times of 0. This is done by changing several unsigned longs	*
 *	back to long.							*
 *									*
 ************************************************************************/
...
#else OLD
/* for speed to avoid calls's - even systemV does this right */
