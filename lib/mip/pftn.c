#ifndef lint
static	char	*sccsid = "@(#)pftn.c	1.8	(ULTRIX)	4/14/86";
#endif lint
/************************************************************************
 *
 ...
 * 011- Bug fix for constant pointers.
 *
 * 	Lu Anne Van de Pas, 02-Mar-86
 * 010- Added support from intializing f floating constants
 *
 *	Victoria Holt, 26-Feb-86
 * 009- Added support for const and volatile.
 *
 *	Rich Phillips, 14-Sep-84
 * 008- I put the "if" in the wrong place for fix 6. It should go at
 *	the end of the switch, not after the default case.
 *
 *	Rich Phillips,  13-Sep-84
 * 007- Back out of fix 004, see mod history in local.c for reason.
 *
 *	Rich Phillips,  07-Sep-84
 * 006- Fix change 003 so previous symbol table entries are not removed
 *	causing "undefined" errors in later function definitions.
 *
 *	Rich Phillips,  31-Jul-84
 * 005- Issue a warning if the user redeclares a function argument at
 *	level 2.
 *
 *	Rich Phillips,  20-Jul-84
 * 004- Set SWASREG flag in the symbol table if a name was defined 
 *	with REGISTER, but changed by the compiler.
 *
 * 	Rich Phillips,  16-Jul-84
 * 003- Issue an error when an arguement is defined but not in the arguement
 *	list of the function def. It wasn't being caught when the name was used
 *	previously for something else that is still accessable.
 *
...
# endif
	...
	/*
		RAP003
		The logic of the following "if" was changed to catch the case
		where we are at level 1 and the previous use of the name is
		not FARG or  UNDEF. An external for example.
	*/
	if( blevel == 1 ){
		if( stp!=FARG ) switch( class ){
		...
		case TYPEDEF:
			if (stp!=UNDEF)
				goto mayhide; /*RAP006 RAP008*/
			;
			}
		goto enter;
		} else if ( stp == UNDEF || stp == FARG ) goto enter;
 mayhide:
	if( type != stp ) goto mismatch;
	if (typattr != stattr) goto mismatch;
...
# endif
	...
	if( blevel > slev && class != EXTERN && class != FORTRAN &&
		...
		/*RAP005
			Issue a warning if a function paramater has just
			been redeclared first thing in the function.
		...
		*/
		if (slev==1 && blevel==2)
#ifndef FLEXNAMES
			werror("immediate redeclaration of argument: %.8s",
				p->sname);
#else
			werror("immediate redeclaration of argument: %s",
				p->sname);
#endif
