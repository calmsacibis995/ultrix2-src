#ifndef lint
static	char	*sccsid = "@(#)order.c	1.3	(ULTRIX)	3/12/86";
#endif lint
...
*
*	Lu Anne Van de Pas, Mar-2,1986
* vdp002 - added support for doing f floating arithmetic in f float 
*	when the fflag is set. 
*
*	Rich Phillips, 7-11-84
* RAP001 - Fix bug where register arrays are accessed with an illegal
*	   register number because someone changed an oreg to a reg. The
*	   problem occurred when structures of length 4 were passed as
*	   parameters. Problem showed up in tshape, fixed in genargs.
*
************************************************************************/
...
genargs( p, ptemp ) register NODE *p, *ptemp; {
	...
	if( p->in.op == STARG ){ /* structure valued argument */
		...
		if( p->in.left->in.op == ICON ){
			...
			p->in.op = UNARY MUL;
			/*RAP001 (first of a couple for this fix)
				Change the type field so indexing will be
				used where possible, otherwise tlen, called
				from oreg2 never knows the true length. This
				improves the code a little. The next change
				avoids the out of bounds reference in tshape.
			*/
			switch (size){
			case 1:
				p->in.type =  CHAR;
		...
		/*RAP001
			Don't go through complicated structure passing if what's
			being passed is 1,2, or 4 bytes long. Better code will
			be used for these cases if we go to ordinary. 

			If we don't, when the length is 4, the oreg will use
			indexing, which will cause an out of bounds reference
			to the register arrays in tshape later because the 
			oreg would be changed to a reg for some reason.
		*/
		if ((size == 1 || size == 2 || size == 4 ) && p->in.op == OREG )
			goto ordinary;
	...
	/* ordinary case */
ordinary:	/* label added in RAP001 change */
