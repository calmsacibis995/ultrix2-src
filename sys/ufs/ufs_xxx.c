#ifndef lint
static char *sccsid = "@(#)ufs_xxx.c	1.2	ULTRIX	10/3/86";
#endif
	...
	struct pt *nptbl;			/* New partition table */
{
	...
	for ( i = 0; i < nblkdev ; i++ ) {
            	...
            	if ( bdevsw[i].d_ioctl == cdevsw[majno].d_ioctl) {
			/*
			 *	Make a bdev with an "a" partition
			 */
	...
	for (mp = &mount[0]; mp < &mount[NMOUNT]; mp++) {
		...
		/*
		 *	See if the "a" partition of the disk is mounted
		 */

		if ( mp->m_dev == bdev && mp->m_bufp != NULL ) {
