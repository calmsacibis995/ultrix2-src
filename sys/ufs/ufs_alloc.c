#ifndef lint
static	char	*sccsid = "@(#)ufs_alloc.c	1.8	(ULTRIX)	1/29/87";
#endif lint
...
/*
 * Allocate an gnode in the file system.
 * 
 * A preference may be optionally specified. If a preference is given
	...
	int mode;
{
	...
	if (gno == 0)
		goto nognodes;
	gp = ufs_gget(pgp->g_dev, pgp->g_mp, gno, 0);
	if (gp == NULL) {
		ufs_gfree(pgp, gno, 0);
		return (NULL);
	}
	if(gp->g_mp->m_fstype != GT_ULTRIX) 
		panic("ufs_galloc: ufs_gget returned wrong fs type");
	if (gp->g_mode) {
...
#ifdef GFSDEBUG
	if(gp->g_mode != 0) {
		printf("ufs_gfree: gp 0x%x mode 0%o should be 0\n", gp, 
		gp->g_mode);
		panic("ufs_gfree");
	}
#endif
