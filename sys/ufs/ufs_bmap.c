#ifndef lint
static	char	*sccsid = "@(#)ufs_bmap.c	1.8	(ULTRIX)	3/3/87";
#endif lint
	...
	if (bn < NDADDR) {
		...
		if (nb == 0 || gp->g_size < (bn + 1) * fs->fs_bsize) {
			...
			if (((gp->g_mode&GFMT) == GFDIR) || sync)
				 ...
				 * them on the disk.
				 *
				 * 003 - bwrite should also be called when
				 * syncronous writes are being performed.
				 */
	...
	for (; j <= NIADDR; j++) {
		...
		if (nb == 0) {
			...
			if (
				 ...
				 *
				 * 003 - bwrite should also be called if
				 * syncronous writes are being performed.
				 */

				bwrite(nbp);
			else
				bdwrite(nbp, (struct gnode *)0);
			bap[i] = nb;

			/*
			 * 003 - Call bwrite if syncronous writes are
			 * being performed.
			 */
