#ifndef lint
static char *sccsid = "@(#)tty.c	1.16	ULTRIX	10/3/86";
#endif
	...
	register struct tty *tp;
{
	...
	if ((tp->t_state&TS_TBLOCK)==0)
	    if (x >= TTYHOG/2 &&
	    ((tp->t_flags&(RAW|CBREAK)) || (tp->t_canq.c_cc > TTYHOG/3))) {
...
ovhiwat:
	 ...
	 * It can also happen it the device routine called by
