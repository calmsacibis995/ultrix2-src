...
#ifndef lint
static char *sccsid = "@(#)sys_trace.c	1.7	ULTRIX	12/16/86";
#endif lint
...
int tracelost = 0;
char *syscallnames[];	/* from syscalls.c */
	...
	register int mode;
{
	...
	if (mode != FREAD || traceopens >= TR_USRS) return(ENXIO);
	for (i=0;i<TR_USRS;i++,trp++) if (trp->open == CLOSED) break;
	if (i >= TR_USRS) return(ENXIO);/* none available */
	trp->bp = geteblk(tracebsize);	/* get a buffer */
	...
	register struct uio *uio;
{
	...
	if (trp->open == CLOSED) return(ENXIO);
	bp = trp->bp;
	...
	register int flag;
{
	...
	if (flag == FREAD && trp->open == OPEN &&
	    trp->bp->b_bcount > (int) (tracebsize * FRACTION)) return(1);
