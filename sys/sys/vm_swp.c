#ifndef lint
static char *sccsid = "@(#)vm_swp.c	1.15	ULTRIX	3/3/87";
#endif
	...
	register char *rout;
{
	...
	uprintf("sorry, pid %d was killed due to no swap space\n", p->p_pid);
	/*
...
static struct abuf *async_bp = (struct abuf *)0;
	...
	int flag;
{
	...
	if((flag&FNBUF) == 0)
		return(0);
	for(dp = async_bp; dp; dp = dp->b_forw) {
		bp = &dp->bp;
		if(bp->b_dev == dev)
			return(1);
	...
	int flags;	/* fp flags */
{
	...
	register struct abuf *dp, *tdp;
	register struct proc *p = u.u_procp;
			
		for(dp = async_bp; dp;  tdp = dp, dp = dp->b_forw) {
			if(dp->bp.b_proc == p) {
				goto cont;
			}
cont:
	...
	for(;found < *count; found++) {
			tdp = 
			(struct abuf *)km_alloc(sizeof (struct abuf), KM_CLRSG);
	...
	register flag;
{
	...
	if (doingasync(dev, flag)) { /* This dev was doing async */
		for(dp = async_bp; dp; dp = dp->b_forw) {
			...
			if(dp == async_bp) /* Advance head of list */
				async_bp = dp->b_forw;
			if(dp == async_bp) /* Last active buffer */
				async_bp = (struct abuf *)0;
			bremhash(dp);
			(void) km_free(dp, sizeof(struct abuf));
