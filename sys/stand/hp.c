...
#ifndef lint
static	char	*sccsid = "@(#)hp.c	1.6		(ULTRIX)	4/18/86";
#endif lint
 ...
 * 
 * 15-Nov-84 -p. keilty
 *	Reset io->i_cc to bytecnt on done because  of ecc success changed
 *	it to complete transfer. Read routine which called strategy needs
 *	i_cc to be the total count transfered.
 * 9-Jan-84 - Tom Tresvik
 *	Moved local struct st from within hpopen and made it global.  This
 *	allows more than one file open to occur.  Fixes problem of having to
 *	reboot after an invalid file is specified the first time.  It also
 *	allows the PCS750 support in boot to work.
 *
 * ---------------------------------------------------------------------
 */
...
found:
		 ...
		 */
		st_open = &hpst[hp_type[unit]];
		tio = *io;
		tio.i_bn = st_open->nspc * st_open->ncyl - st_open->nsect;
	...
	if (io->i_boff < 0 || io->i_boff > 7 ||
	    st_open->off[io->i_boff]== -1)
		_stop("hp bad minor");
	io->i_boff = st_open->off[io->i_boff] * st_open->nspc;
}
...
success:
	...
	io->i_cc = bytecnt;   /* reset i_cc to total count transfered */
	return (bytecnt);
}
