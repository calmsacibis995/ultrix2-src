#ifndef lint
static char *sccsid = "@(#)if_pcl.c	1.10	ULTRIX	10/3/86";
#endif lint
	...
	int unit, uban;
{
	...
	if (unit >= nNPCL || (ui = pclinfo[unit]) == 0 || ui->ui_alive == 0 ||
	    ui->ui_ubanum != uban)
		return;
	printf(" pcl%d", unit);
	pclinit(unit);
