...
#ifndef lint
static char *sccsid = "@(#)if_de.c	1.22	ULTRIX	3/3/87";
#endif lint
...
/*001*/                 	if ((rp->r_flags & XFLG_MTCH) && !(ds->ds_if.if_flags & IFF_LOOPBACK)) {
	...
	if (csr0 & PCSR0_RCBI) {
		mprintf("de%d: buffer unavailable\n", unit);
		addr->pclow = PCSR0_INTE|CMD_PDMD;
	}
}
	...
	struct sockaddr *dst;
{
	...
	default:
		 ...
		 */
		if (ifp->if_flags & IFF_LOOPBACK) {
			if ((m->m_len > DELUA_LOOP_LEN) && (ds->ds_devid == DELUA)) {
				return(EINVAL);
			}
		}
		/*
