...
#ifndef lint
static char *sccsid = "@(#)if_acc.c	1.9	ULTRIX	10/3/86";
#endif lint
	...
	int unit, uban;
{
	...
	if (unit >= nNACC || (ui = accinfo[unit]) == 0 || ui->ui_alive == 0 ||
	    ui->ui_ubanum != uban)
		return;
	printf(" acc%d", unit);
	sc = &acc_softc[unit];
