#ifndef lint
static char *sccsid = "@(#)if_un.c	1.9	ULTRIX	10/3/86";
#endif lint
...
#include "un.h"
#if NUN > 0 || defined(BINARY)
/*
 * Ungermann-Bass network/DR11-W interface driver
 */

#include "../data/if_un_data.c"
	...
	int unit, uban;
{
	...
	if (unit >= nNUN || (ui = uninfo[unit]) == 0 || ui->ui_alive == 0 ||
	    ui->ui_ubanum != uban)
		return;
	printf(" un%d", unit);
	uninit(unit);
	...
	int unit;
{
	...
	case US_READ:
		...
		case UNTYPE_INQUIRE: {
			struct sockaddr_in *sin;
#ifdef ultrix1.1 /* obviously this driver does not work anymore */
			us->us_if.if_host[0] =
