#ifndef lint
static	char	*sccsid = "@(#)tcp_timer.c	1.4	(ULTRIX)	12/9/85";
#endif lint
...
tcp_fasttimo()
{
	...
	for (; inp != &tcb; inp = inp->inp_next)
		if ((tp = (struct tcpcb *)inp->inp_ppcb) &&
			...
			/* 
			 * check pointers after tcp_output because
			 * TCP_LOOPBACK code will cause tcp_close to
			 * to be called which will call in_pcbdetatch
			 * which will cause inp to be removed from
			 * the queue
			 */
			inpnxt = inp->inp_next; 
			(void) tcp_output(tp);
