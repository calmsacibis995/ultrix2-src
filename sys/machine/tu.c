...
struct tu {
	...
	int	tu_toerrs;	/* count of timeout errors */
...
tus_get:
		if (tudata.pk_flag == TUF_DATA) {
			...
			if (tudata.pk_mod > 1) {        /* hard error */
				...
				printf("tu%d: hard error bn%d, pk_status %o %s\n", minor(bp->b_dev)&DNUM, bp->b_blkno, tudata.pk_mod&0377, sp);
...
bad:
		if (c == TUF_INITF) {
			mprintf("tu%d: protocol error, state=%s op=%x cnt=%d \
				block=%d\n", (int)tucmd.pk_unit,
				tustates[tu.tu_state], tucmd.pk_op,
				tucmd.pk_count, tucmd.pk_block);

			/* resync protocol and retry ---- pmk */
			tureset();

		} else {
			mprintf("tu%d: receive state error, state=%s byte=%x\n",
			    (int)tucmd.pk_unit,tustates[tu.tu_state],c&0xff);
			if (tutab.b_actf)
				tu_restart();
			else
				wakeup((caddr_t)&tu);
...
tuwatch()
{
	...
	printf("tu%d: timeout, %s\n", tucmd.pk_unit,
		tu.tu_toerrs < MAXTOERRS ? "command restarted" : "give up!");
#ifdef TUDEBUG
...
tu_restart()
{
	...
	mtpr(CSTS, IE);
	tu.tu_state = TUS_IDLE;
	timeout(tustart, (caddr_t)0, hz * 4);
	splx(s);
}
