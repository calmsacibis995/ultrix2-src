...
#ifndef lint
static char *sccsid = "@(#)ka610.c	1.14	ULTRIX	10/3/86";
#endif lint
...
struct cpusw *cpup;
{
	 ...
	 * so we fake it.
	 */
	nxv=zmemall(memall,sizeof(struct nexus));
	/*
	 * check contiguious, get physical address
	 * make use of knowledge about how zmemall allocates
	 */
	nxp =  (char *)contigphys( nxv, sizeof(struct nexus),
		&Usrptmap[btokmx( (struct pte *) nxv)]);
	/*
	 * Map the nexus.
	 */
	...
	do {
		if( !(*cpup->v_badaddr)( qmemcsrbase, sizeof(short) ) )
			*qmemcsrbase++ = 1;
