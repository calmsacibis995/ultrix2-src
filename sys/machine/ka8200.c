#ifndef lint
static char *sccsid = "@(#)ka8200.c	1.36	ULTRIX	2/12/87";
#endif lint
...
struct cpusw *cpup;
{
	...
	bidata[0].biintr_dst = 1 << bicpunode;
	...
	for (binode = 0; binode < NBINODES; binode++, nxv++) {
		...
		if ((*cpup->v_badaddr)((caddr_t) nxv, sizeof(long)) == 0) {
	...
	if (cpucount ==1 ) cpu_subtype = ST_8200;
	...
	procfound = 0;
	probebi(cpup,0);
	/* loop for all possible nodes and start and secondary cpus we saw */
	for (nexndx = 0; nexndx < NBINODES; nexndx++) {
		if ((procfound & (1 << nexndx)) != 0) {
			/* only start slaves if configured */
			if (activecpu < maxcpu) ka820procinit(nexndx);
			else mprintf("WARNING -- 2nd CPU not configured \n");
...
ka820init(nxv,nxp,cpup,binumber,binode)
char *nxp;
char *nxv;
struct cpusw *cpup;
int binumber;
int binode;
{
	...
	((struct ka820_regs *) nxv)->ka820_biic.biic_ip_msk = 0xffff0000;
...
int nexid;
{
		...
		} else if (ka820txstr(nexid, "I\r") == 0) {
...
ka820rxcd()
{
	...
	if ((1 << binode) == bidata[0].biintr_dst) {
...
caddr_t cmcf;
{
	...
	if (cold == 1) {
		...
		logmck((int *)cmcf, mfpr(BINID), recover);
		if (recover == 0) {
			cprintf("\tsumpar\t= %x\n", mcf->mc8200_summary);
			cprintf("\tparm1\t= %x\n", mcf->mc8200_parm1);
			cprintf("\tva\t= %x\n", mcf->mc8200_va);
