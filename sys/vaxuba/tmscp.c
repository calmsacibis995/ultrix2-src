#ifndef lint
static char *sccsid = "@(#)tmscp.c	1.57      ULTRIX  1/30/87";
#endif lint
 ...
 *
 * 30-Jan-87	-robin
 *	added a timer routine.  It is called from start() and will cause
 *	start() to be run after 1 sec.  This will prevent raw i/o hangs
 *	if start() asks for resources and does not get them.  Without
 *	other i/o going on start() is never re-entered if it returns without
 *	compleating the i/o request, the timer forces start() to be re-entered.
 *
 * 04-Nov-86  - lp
 *	Fixed start routine to return error if a tape mark is seen.
 *
 * 30-Oct-86  - robin
 *	Fixed strategy to return error if a tape mark was seen and there are
 *	still outstanding reads comming in.
 *
 * 16-Oct-86  - robin
 *	Fixed the following:
 *		1). On read, if end of tape (EOT) is seen at anytime
 *		    the EOM is set in tms_info[].tms_flags. (as per Larry P.)
 *		2). If an online failed and was followed by another
 *		    online which would succeed, it second fails because
 *		    the timer in the first online does a wakeup on the second online.
 *		3). Setting cache off BOT was failing because the format field
 *		    was non zero.  The tape density (format) can be set only at
 *		    BOT and must be a zero when not at BOT; Zero off BOT is a noop
 *		4). Cache is now set when the drive comes online and sets density
 *		    The drive will cache (if it can) by default and an ioctl is used to
 *		    turn it off when necessary.
 *
 * 22-Sept-86 - jaw 
 *	fix tu81-E hanging on booting.
 *
 * 10-Sept-86 - lp
 *	Undid part of 9-sept-86. Let all pending ops fail on TM. This
 *	will guarantee that we never get past EOT.
 *
 *  9-Sept-86 - Robin
 *	Added clear serious exception mod to all read operations and changed the
 *	responce routine to again report serious exceptions.
 *
 *  4-Sep-86 -- Robin
 *	cache ioctl is a now a no-op if the unit does not have caching.
 *
 ...
 *      for bcopy's.
 *
 * 18-Aug-86 - Robin Lewis
 *      Changed thr tmscpreset control loop to use the user supplied
 *      number of controlers and not what UEG compiled in at build time.
 *
 * 14-Aug-86 - Robin
 *      Put in ECO changes to tmscp that are required for TK70 support.
 *      these are required for media dependent density.
 *
 * 22-July-86 - Robin
 *
 *      Fixed a bug in the ioctl which caused variables to be
 *      initalized wrong when they are declared.
 *
 ...
 *
 * 11-Jun-86 - bjg
 *      Add error logging for tmscp errors.
 *
 * 1-May-86 - robin
 *      Made bug fix to stop the change of tape density at any point other
 *      than BOT.  This showed up when putting multi. files
 *      on one low density tape because it reset the density on every
 *      open (which is an invald operation). Changes were made in the open
 *      function.
 *
 * 28-Apr-86 - afd
 *      Added a Get Unit Status Command to the open routine so that
 *      If the drive has been taken offline since the last access we
 *      can detect that its offline in the open routine.
 *
 * 22-Mar-86 - lp
 *      N-buffered fix got lost someplace in here!
 *
...
#define TMS_CLX         11              /* clear hard/soft-ware problem */
#define TMS_CLS         12              /* clear subsystem */
#define TMS_ENAEOT      13              /* enable default eot code */
#define TMS_DISEOT      14              /* disable default eot code */
/* These go last: after all real mt cmds, just bump the numbers up */
#define TMS_LOWDENSITY  15              /* set unit to low density */
#define TMS_HIDENSITY   16              /* set unit to high density */
...
#       ifdef DEBUG
        i=1;
#       endif DEBUG
        while(sc->sc_state != S_RUN && sc->sc_state != S_IDLE);   /* wait */
        ...
        *((long *) mp->mscp_dscptr ) |= TMSCP_OWN | TMSCP_INT;/* maybe we should poll*/
        i = tmscpaddr->tmscpip;
        /*
         * Wait for some status: This assumes that we get
         * a non-zero status, like Available or Offline.
         */
        while(!tms_info[ui->ui_unit].tms_status)
                ;
#       ifdef DEBUG
        printd("tmscpslave: status = %o\n",tms_info[ui->ui_unit].tms_status );
#       endif
        tmscpip[ui->ui_ctlr][ui->ui_slave] = 0;
        if(tms_info[ui->ui_unit].tms_status == M_ST_OFFLN)     /* packet from a GTUNT */
...
#       endif lint
        ...
        bcopy(mscpdev_temp,tms_info[ui->ui_unit].tms_device,
              strlen(mscpdev_temp));
        tms_info[ui->ui_unit].tms_softcnt = 0;    /* set to zero */
        tms_info[ui->ui_unit].tms_hardcnt = 0;    /* set to zero */
...
#       ifdef DEBUG
         ...
         */
        if((tms_info[ui->ui_unit].tms_status & M_ST_MASK) != M_ST_AVLBL)
...
#                               endif DEBUG
                ...
                tmscpaddr->tmscpsa = ((int)&sc->sc_tmscp->tmscp_ca.ca_ringbase)
                        | (((um->um_hd->uba_type & UBA780)!=0) ? TMSCP_PI : 0);
                sc->sc_state = S_STEP2;
                return;
...
#       endif DEBUG
        ...
        /*
         * Driver may believe that tape drive is still online, however tape
        ...
        *((long *) mp->mscp_dscptr ) |= TMSCP_OWN | TMSCP_INT;
        i = tmscpaddr->tmscpip;
        timeout(wakeup,(caddr_t) mp->mscp_cmdref,30 * hz);
        sleep((caddr_t) mp->mscp_cmdref,PSWP+1);
        if ((tms_info[ui->ui_unit].tms_status & M_ST_MASK) == M_ST_OFFLN)
		...
		{
                        tms_info[ui->ui_unit].tms_position = 0;/* init to BOT */
                }
                                            /* need to sleep on something */
#               ifdef DEBUG
...
#               endif DEBUG
                *((long *) mp->mscp_dscptr ) |= TMSCP_OWN | TMSCP_INT;
                 ...
                 * can take a few minutes.  This is done in 8 30 sec.
		 * timeouts because the wakeup sometimes is not done
		 * when the online rsp. is recieved but times out later.
                 */
		interval=0;
		tms_info[ui->ui_unit].tms_type=0;
		while((interval < 8) && (tms_info[ui->ui_unit].tms_type == 0))
		{
	                timeout(wakeup,(caddr_t) mp->mscp_cmdref,30 * hz);
	                sleep((caddr_t) mp->mscp_cmdref,PSWP+1);
			interval++;
        ...
        if ((mp->mscp_flags & M_EF_EOT) && (dis_eot_tms[unit] != DISEOT))
        {
                tms_info[ui->ui_unit].tms_flags &= DEV_EOM;
        }
        else 
	{
                tms_info[ui->ui_unit].tms_flags = 0;
        }


        if(ui->ui_flags == 0)
                {
                tms_info[ui->ui_unit].tms_flags |= DEV_OFFLINE;
                if(!(flag & FNDELAY))
                        {
                        DEV_UGH(tms_info[ui->ui_unit].tms_device,
                                unit,"offline");
        ...
        /*
         * We also want to do a Get Unit Status to fill in correct 
	 * data now that the drive is online.
         *
	 * If the drive is on line and will cache turn it on
	 * if it's not already on
	 */
	if ( (tms_info[ui->ui_unit].tms_flags & DEV_OFFLINE) == 0)
	{
	        s = spl5();
       		while(0 ==(mp = tmscpgetcp(um)))
	        {
         ...
         * Only set the density if at BOT (position 0).
         */

#       ifdef DEBUG
        printd("sel = 0x%x\n",sel);
#       endif DEBUG
        if(tms_info[ui->ui_unit].tms_position == 0 &&
           !(tms_info[ui->ui_unit].tms_flags & DEV_OFFLINE)) {
...
#       endif
        ...
        if(tms_info[ui->ui_unit].tms_flags & DEV_OFFLINE) {
                goto offline;
        }
        /* Allow us to write tape marks/issue rewind */
        if(tms_info[ui->ui_unit].tms_flags & DEV_EOM) {
                tms_info[ui->ui_unit].tms_flags |= DEV_CSE;
                tms_info[ui->ui_unit].tms_serex = 2;
        }
        if (flag == FWRITE || ((flag&FWRITE) &&
            (tms_info[ui->ui_unit].tms_flags & DEV_WRITTEN)))
                {
        ...
        register struct uba_ctlr *um;
{
        ...
        int i,tempi,unit,s,tmscp_timer();
	extern int hz;
	...
	/*
	 * If a tape mark was seen then don't do any more reads because
	 * buffered i/o may have reads outstanding.
         */
	if(  (tms_info[ui->ui_unit].tms_serex == 2)
	&&   (bp->b_flags & B_RAWASYNC)
	&&   (bp->b_flags & B_READ) )
	{
		bp->b_flags |= B_ERROR;
		bp->b_error = EIO;
		/* Unlink buffer and continue */
        ...
        if (bp == &ctmscpbuf[um->um_ctlr])
                ...
                if ((i = ubasetup(um->um_ubanum, bp, i)) == 0)
                        {
#                       ifdef DEBUG
                        printd("um->um_ubanum=%d bp=%d i=%d\n",um->um_ubanum,bp,i);
                        printd("tmscpstart: GTUNT %d ubasetup = %d\n",ui->ui_unit, i);
                        if(tmscpdebug)DELAY(10000);
#                       endif DEBUG
			/* only time once per controller */

			if(sc->sc_timer != 1)
			{
				sc->sc_timer = 1;
				timeout(tmscp_timer,um,hz);
			}
                        break;
                        }
        if (um->um_hd->uba_type&(UBABUA|UBA750))
...
#               endif DEBUG
			 ...
			 * and if the drive has cache set it to on as we come
			 * up on it.
                         */
#                       ifdef DEBUG
                                printd("Entered the ioctl LOWDENSITY code\n");
#                       endif DEBUG
                        mp->mscp_opcode = M_OP_STUNT;
                        mp->mscp_unitflgs = tms_info[ui->ui_unit].tms_unitflgs;
                        mp->mscp_mediaid = 0;   /* default device dependant parameters */
			if ( (tms_info[ui->ui_unit].tms_unitflgs & M_UF_CACH)
			&& ((tms_info[ui->ui_unit].tms_unitflgs & M_UF_WBKNV)==0))
			{
				tms_info[ui->ui_unit].tms_unitflgs |= M_UF_WBKNV;
				mp->mscp_unitflgs = tms_info[ui->ui_unit].tms_unitflgs;
			}
                        /* To set the density to LOW first zero the density part
                         * of theflags field and switch on the type of media.
                         * Then look in the menu for what the vvalid values
                         * are, starting from the low density to the highest, and
                         * set the density to the first valid value found.
                         * By default the setting the flags field to zero
                         * will set the density to some hardware default density.
                         */
#                       ifdef DEBUG
                                printd("format is o%o\n",tms_info[ui->ui_unit].tms_format);
                                printd("menu is o%o\n",tms_info[ui->ui_unit].tms_fmtmenu);
#                       endif DEBUG
                        tms_info[ui->ui_unit].tms_format &= 0xff00;
                        tms_info[ui->ui_unit].tms_category_flags = 0;
                        switch (tms_info[ui->ui_unit].tms_format)
                        {
                        case M_TC_9TR:
                                if((tms_info[ui->ui_unit].tms_fmtmenu & M_TF_800) != 0)
                                {
                                        tms_info[ui->ui_unit].tms_category_flags |= DEV_800BPI;
                                        tms_info[ui->ui_unit].tms_format |= M_TF_800;
                                }
                                else
                                if((tms_info[ui->ui_unit].tms_fmtmenu & M_TF_PE) != 0)
                                {
                                        tms_info[ui->ui_unit].tms_category_flags |= DEV_1600BPI;
                                        tms_info[ui->ui_unit].tms_format |= M_TF_PE;
                                }
                                else
                                if((tms_info[ui->ui_unit].tms_fmtmenu & M_TF_GCR) != 0)
                                {
                                        tms_info[ui->ui_unit].tms_category_flags |= DEV_6250BPI;
                                        tms_info[ui->ui_unit].tms_format |= M_TF_GCR;
                                }
                                else
                                if((tms_info[ui->ui_unit].tms_fmtmenu & M_TF_T82) != 0)
                                {
                                        tms_info[ui->ui_unit].tms_category_flags |= DEV_6250BPI;
                                        tms_info[ui->ui_unit].tms_format |= M_TF_T82;
                                }
                                else
                                {
                                        tms_info[ui->ui_unit].tms_category_flags |= DEV_6250BPI;
                                        tms_info[ui->ui_unit].tms_format = M_TC_KLUDGE;
                                }
                                break;
                        case M_TC_RV80:
                                tms_info[ui->ui_unit].tms_format |= M_TF_NOR;
/*                              tms_info[ui->ui_unit].tms_category_flags |= DEV_6250BPI;*/
                                break;
                        case M_TC_TK50:
                                if((tms_info[ui->ui_unit].tms_fmtmenu & M_TF_NOR) != 0)
                                {
                                        tms_info[ui->ui_unit].tms_category_flags |= DEV_6666BPI;
                                        tms_info[ui->ui_unit].tms_format |= M_TF_NOR;
                                }
                                else
                                {
                        ...
                        case M_TC_KLUDGE:
                                ...
                                if((tms_info[ui->ui_unit].tms_fmtmenu & M_TF_BLK) != 0)
                                {
                                        tms_info[ui->ui_unit].tms_category_flags |= DEV_6666BPI;
                                        tms_info[ui->ui_unit].tms_format |= M_TF_BLK;
                                }
                                else
                                {
                                        tms_info[ui->ui_unit].tms_category_flags |= DEV_6250BPI;
...
#                       ifdef DEBUG
                                printd("mp->mscp_format = %o\n",mp->mscp_format);
#                       endif DEBUG
                        break;
                case TMS_HIDENSITY:
                        /* To set the density to HIGH first zero the density part
                         * of theflags field and switch on the type of media.
                         * Then look in the menu for what the valid values
                         * are, starting from the high density to the lowest, and
                         * set the density to the first valid value found.
                         * By default, setting the flags field to zero
                         * will set the density to some hardware default density.
			 * Also set the cache to on if the drive has caching.
                         */
                        mp->mscp_opcode = M_OP_STUNT;
                        mp->mscp_unitflgs = tms_info[ui->ui_unit].tms_unitflgs;
                        mp->mscp_mediaid = 0;   /* default device dependant parameters */
...
/*                                      tms_info[ui->ui_unit].tms_category_flags |= DEV_6250BPI;*/
                        ...
                        case M_TC_RV80:
                                tms_info[ui->ui_unit].tms_format |= M_TF_NOR;
/*                              tms_info[ui->ui_unit].tms_category_flags |= DEV_XXXXBPI;*/
                                break;
                        case M_TC_TK50:
                                if((tms_info[ui->ui_unit].tms_fmtmenu & M_TF_BHD) != 0)
                                {
                                        tms_info[ui->ui_unit].tms_category_flags |= DEV_10240BPI;
                                        tms_info[ui->ui_unit].tms_format |= M_TF_BHD;
                                }
                                else
                                {
                                        tms_info[ui->ui_unit].tms_category_flags |= DEV_6666BPI;
        ...
        int i;
{
        ...
        if ((mp->mscp_header.tmscp_credits & 0xf0) > 0x10)      /* Check */
                return;
#       ifdef DEBUG
        printd("tmscprsp: opcode 0%o status 0%o subcode 0x%o ",mp->mscp_opcode,mp->mscp_status&M_ST_MASK,(mp->mscp_status >> 5));
	printd("mp->mscp_status = 0%o \n",mp->mscp_status);
#       endif DEBUG
        /*
...
#                       endif DEBUG
        ...
        if (tms_info[ui->ui_unit].tms_clserex != 1)
                ...
                tms_info[ui->ui_unit].tms_status = mp->mscp_status;
                }
        else tms_info[ui->ui_unit].tms_clserex = 0;

        if (mp->mscp_unitflgs & M_UF_WRTPH) {
                tms_info[ui->ui_unit].tms_flags |= DEV_WRTLCK;
        }
        ...
        case M_OP_ONLIN|M_OP_END:
                ...
                if (st == M_ST_SUCC)
                        ...
                        tms_info[ui->ui_unit].tms_dsize=(daddr_t)mp->mscp_maxwrt;
#                       ifdef DEBUG
...
#                       endif
                        ...
                        {
                        if(dp->b_actf) {
                            if(ui->ui_flags) {    /* online with error */
...
#               endif DEBUG
			...
			if(bp->b_ubinfo == 0)
			{
                ...
                if (st == M_ST_OFFLN || st == M_ST_AVLBL)
                        ...
                        if (mp->mscp_flags & M_EF_EOT) 
				tms_info[ui->ui_unit].tms_flags |= DEV_EOM;
                if (st != M_ST_SUCC)
                        {
                        if (mp->mscp_flags & M_EF_SEREX && !tms_info[ui->ui_unit].tms_serex)
                        ...
                        if (mp->mscp_flags & M_EF_EOT) {
                            if((bp->b_flags&B_READ) == 0) {
                                ...
                                tms_info[ui->ui_unit].tms_flags |= DEV_EOM;
                        } else if (st != M_ST_TAPEM)
                                {
                                if( tms_info[ui->ui_unit].tms_serex) {
				if( tms_info[ui->ui_unit].tms_serex != 2 ||
					((bp->b_flags&B_RAWASYNC) == 0)) {
                                harderr(bp, "tms");
...
#                               endif DEBUG
                ...
                if (mp->mscp_opcode != (M_OP_AVAIL|M_OP_END) &&
                    mp->mscp_opcode != (M_OP_STUNT|M_OP_END))
                        tms_info[ui->ui_unit].tms_position = mp->mscp_lbn;
                iodone(bp);
                break;
...
#               ifdef DEBUG
                ...
                printd("tmscprsp: unit %d mediaid %x %c%c %c%c%c%d %x %x t=%d m=%x\nunitflgs ox%x\n"
        ...
        register struct buf *bp;
{
        ...
        um = ui->ui_mi;
#       ifdef DEBUG
        printd("strategy: bp = %x\nbp->bcount = %x\n",bp,bp->b_bcount);
#       endif DEBUG

        if ((tms_info[ui->ui_unit].tms_flags & DEV_EOM) &&
            !((tms_info[ui->ui_unit].tms_flags & DEV_CSE) ||
            (dis_eot_tms[unit] & DISEOT))) {
        ...
        dev_t dev;
{
        ...
        if (unit >= nNTMS)
                return (ENXIO);
        ui = PHYS(tmsdinfo[unit], struct uba_device *);
        if (ui->ui_alive == 0)
                return (ENXIO);
        ...
        DELAY(2000000);
        tmscpp = PHYS(&tmscpd[ui->ui_ctlr], struct tmscp *);
        ...
        int flag;
{
        ...
        if (unit >= nNTMS)
                return (ENXIO);
        ui = tmsdinfo[unit];
        sc = &tmscp_softc[TMSCPCTLR(dev)];
        bp = &ctmscpbuf[TMSCPCTLR(dev)];
        ...
        case MTIOCTOP:  /* tape operation */
                ...
                case MTCACHE:
			tms_info[ui->ui_unit].tms_flags &= ~DEV_EOM;
			if((tms_info[ui->ui_unit].tms_unitflgs & M_UF_CACH) == 0)
				return(0);  /* No cache support on this drive */
			if((tms_info[ui->ui_unit].tms_unitflgs & M_UF_WBKNV))
				return(0); /* Cache support set on this drive */
			callcount = 1;
			...
			if((tms_info[ui->ui_unit].tms_unitflgs & M_UF_CACH) == 0)
				return(0);  /* No cache support on this drive */
			if((tms_info[ui->ui_unit].tms_unitflgs & M_UF_WBKNV) == 0)
				return(0); /* Cache not set on this drive */
			callcount = 1;
			fcount = 1;		/* wait for this rewind */
			break;
        ...
        case MTIOCGET:
                ...
                mtget->mt_erreg = tms_info[ui->ui_unit].tms_status & M_ST_MASK;
        ...
        case DEVIOCGET:                         /* device status */
                ...
                } else if (ui->ui_hd->uba_type&UBABLA) {
                        devget->bus = DEV_BI;
                ...
                case DEV_BI:
/* Need code to determine if interface is KLESI or AIE (assume KLESI) */
                        bcopy(DEV_KLESI,devget->interface,
                              strlen(DEV_KLESI));
                        break;
                case DEV_CI:
/* Need code to determine if interface is HSC??, HSB?? */
                        break;
                case DEV_UB:
/* Need code to determine if interface is KLESI or TUK50 (assume KLESI) */
                        bcopy(DEV_KLESI,devget->interface,
                              strlen(DEV_KLESI));
                        break;
                case DEV_QB:
                        bcopy(DEV_TQK50,devget->interface,
                              strlen(DEV_TQK50));
                        break;
                }

                bcopy(tms_info[ui->ui_unit].tms_device,
                      devget->device,
                      strlen(tms_info[ui->ui_unit].tms_device)); /* tu81,tk50,...*/
                devget->ctlr_num = ui->ui_ctlr;         /* which interf.*/
                devget->slave_num = ui->ui_slave;       /* which plug   */
                bcopy(ui->ui_driver->ud_dname,
                      devget->dev_name,
                      strlen(ui->ui_driver->ud_dname)); /* Ultrix "tms" */
                devget->unit_num = unit;                /* which tms??  */
                devget->soft_count =
                    tms_info[ui->ui_unit].tms_softcnt;  /* soft er. cnt.*/
                devget->hard_count =
                    tms_info[ui->ui_unit].tms_hardcnt;  /* hard er. cnt.*/
                devget->stat =
                    tms_info[ui->ui_unit].tms_flags;    /* status       */
                devget->category_stat =
                    tms_info[ui->ui_unit].tms_category_flags;   /* c.st.*/
                break;
...
#       endif DEBUG
        ...
        case M_FM_BUSADDR:
                ...
                devtype = (mp->mslg_cntid.val[1] >> 16) & 0xF;
        ...
        case M_FM_STIFEL:
                ...
                class = ELCT_TAPE;      /* Tape class                   */
                type = ELDEV_MSCP;      /* MSCP tape type               */
        ...
        elp->el_body.elbdev.eldevhdr.devhdr_csr = TPHYS(um->um_addr);
        elp->el_body.elbdev.eldevdata.tmslg = *(mp);
        EVALID(elp);


}
/* The timer routine is called via timeout and insures that the
   buffers (if any) are processed.  Timeout is used when we can not
   get resources and must wait for them.
 */
tmscp_timer(um)
	struct uba_ctlr *um;
{
	int s;
	tmscp_softc[um->um_ctlr].sc_timer = 0;
	s = spl5();
	(void) tmscpstart(um);
	splx(s);
}
