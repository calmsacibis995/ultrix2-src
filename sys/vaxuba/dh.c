#ifndef lint
static char *sccsid = "@(#)dh.c	1.15	ULTRIX	12/16/86";
#endif lint
	...
	struct uba_device *ui;
{
	...
	dhdefaultCAR[ui->ui_unit] = ui->ui_flags;
}
	...
	dev_t dev;
{
	...
	unit = minor(dev);
	dh = unit >> 4;
	if (unit >= nNDH*16 || (ui = dhinfo[dh])== 0 || ui->ui_alive == 0)
		return (ENXIO);
	tp = &dh11[unit];
	...
	int flag;
{
	...
	dhsoftCAR[dh] |= (1<<(unit&0xf)) & dhdefaultCAR[dh];
	ttyclose(tp);
}
