#ifndef lint
static char *sccsid = "@(#)sdc.c	1.15	ULTRIX	3/18/87";
#endif lint
...
int sd_delay = 0;  /* touch this variable to cause small delays */
	...
	int flag;
{
	 ...
	 * since it can be either RX50 or RX33 diskette */
	...
	register struct buf *bp;
{
	...
	if( (sd_st.sd_type[unit] == DT_RX50) || (sd_st.sd_type[unit] == DT_RX33) ){ 
		if(dp->b_actf == 0)
			dp->b_actf = bp;
		else
...
#define b_cylin b_resid
	...
	if(sdcbuf.b_active == 0) {
		 ...
		 * 16K buffer */
		vs_bufctl(VS_SDC,VS_ALLOC,VS_DONTWANT);
	}
	splx(s);
	return;
...
sdstart()
{
	...
	if(bp->b_flags & B_BAD) {
			...
			bbr_sleep = 0;
			dp->b_actf = bp->av_forw;
		        vs_bufctl(VS_SDC,VS_DEALLOC,VS_STILLWANT);
			return(0);
	}
#endif BBR_TST
	...
	short flag;
{
	...
	if(sd_st.sd_drno == DRV_NUM2) {
		...
		} else if(sd_st.sd_type[sd_st.sd_drno] == DT_RX33) {
			sdaddr->dkc_reg = (RT_CNT | RT_INVRDY | RT_MOTOR); /* UDC_RTCNT */
			sd_delay++;
			sdaddr->dkc_reg = (MOD_HD | MOD_CHKCRC | MOD_SRTRXH);  /* UDC_MODE */
...
short type;
{
	...
	for(retry=0; retry< 10; retry++) {
		...
		sd_delay++; /* to make sure status is correct */
		status = sdaddr->dkc_stat;
		if( (status & DKC_TERMCOD) != DKC_SUCCESS) {
	...
	int star;
{
		    ...
		    if ( (udc_cstat & CST_ECCER)  ||
			 (udc_cstat & CST_CMPER) ){
			    ...
			    if( (sd_st.sd_type[sd_st.sd_drno] == DT_RX33) ||
				 (sd_st.sd_type[sd_st.sd_drno] == DT_RX50) || xbnflag ) 
				 err++;
			    else if(sd_retry(0)) 
				err++;
	...
	if(sd_st.sd_bleft > 0) {
		...
		vs_bufctl(VS_SDC,VS_DEALLOC,VS_STILLWANT);  /*sdustart() gets called from vs_bufctl*/
	}
	return;
...
/* Initialize the common data for both RX50 and RX33 */
...
int vrfy_flag;     /* flag to decide if block to be replaced */
{
	...
	for(i=0; i<sd_st.sd_nsect; i++) {
		...
		if( rdwr_poll(block,buf0,1,i,cmd,RTRY_CNT,frag_count) ) { 
	...
	char *buffer;
{
    ...
    cprintf("rd_rct: RCT read failed\n");
    return( -1 );
}
