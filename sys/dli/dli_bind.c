#ifndef	lint
static char *sccsid = "@(#)dli_bind.c	1.11	ULTRIX	1/15/87";
#endif	lint
	...
	switch (so_addr->dli_substructype)
	{
		case DLI_802:
			...
			if(sap == SNAP_SAP)
			{
				...
				if(error = osi_ena_802pi(so_802, ifp))
				{
					return(error);
				}
			}
		...
		case DLI_ETHERNET:
			...
			if( error = eaddr_reserved(&so_addr->choose_addr.dli_eaddr, ifp) )
			{
				return(error);
			}
			uentry->dli_sockopt.dli_state = DLS_ON;
 ...
 *
 * Inputs:
 *	eaddr		= Pointer to the structure containing Ethernet addr info.
 */
eaddr_reserved( eaddr, ifp )
