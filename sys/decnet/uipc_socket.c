#ifndef lint
static	char	*sccsid = "@(#)uipc_socket.c	1.2	Ultrix	12/10/84";
#endif lint
	...
	register struct mbuf *m;
{
	...
	if (level == SOL_SOCKET) {
		...
		prp = pffindproto(so->so_proto->pr_family, level);
		if (prp) {
			return ((*prp->pr_usrreq)(so, PRU_SETSOCKOPT,
	...
	register struct mbuf *m;
{
	...
	if (level == SOL_SOCKET) {
		...
		prp = pffindproto(so->so_proto->pr_family, level);
		if (prp) {
			return ((*prp->pr_usrreq)(so, PRU_GETSOCKOPT,
				m, (struct mbuf *)optname, (struct mbuf *)0));
		}
		return (EPROTONOSUPPORT);
	}
