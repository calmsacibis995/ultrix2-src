...
struct nfsrdresult {
	...
	u_long		rr_bufallocsize;	/* size of reponse buffer needed to free */
	char 		*rr_bufallocaddr; 	/* where the buffer has been allocated */
	union {
...
struct nfsrddirres {
	...
	u_long		rd_bufallocsize;/* size of reponse buffer needed to free */
	char		*rd_bufallocaddr;/* where the buffer has been allocated */
...
/* Sun's multiple use of fs_bsize has caused problems. */ 
#define fsstat_bsize	fs_u.fs_fsok_u.fsok_bsize
