 ...
 */
#ifndef lint
static char *sccsid = "@(#)ultrixload.c	1.5	ULTRIX	10/3/86";
#endif lint
/************************************************************************
 *									*
 *			Copyright (c) 1986 by				*
...
main () {
	...
	if (desc.un.d.x.a_magic == 0407) {
		...
		beg_data = i - desc.un.d.x.a_text;
		addr = (char *) (i + beg_data);
	}
	if ((*READ)((char *) 0, i) != i)
		goto shread;
	if (desc.un.d.x.a_magic != 0407) {
		bcopy(beg_data,i,beg_data);
		desc.un.d.x.a_data -= beg_data;
