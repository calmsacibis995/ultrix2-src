...
_setup:
        ...
        ashl    $-24,_cpu,_cpu          # move the cpu type down and 
                                        # throw the rest away
	cmpb	$MVAX_II,_cpu		# Is the a MVAX II chip
	bneq	1f			# No, skip ahead
...
/*
 * Functional Discription:
 *	This routine is relocates the running image's text and data segments
 ...
 *	SET UP TO HANDLE MVAX I AND MVAX II, ONLY.
 *
 * Inputs:
...
8:      movl    r4,(r2)                 # restore good mchkvec
        ...
        .align  2
9:
        casel   _cpu,$1,$VAX_MAX        # check cpu type to clean up
0:
        .word   1f-0b                   # 1 is 780
        .word   1f-0b                   # 2 is 750
        .word   1f-0b                   # 3 is 730
