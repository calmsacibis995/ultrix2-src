...
#ifndef lint
static	char	*sccsid = "@(#)mstr.c	1.1	(ULTRIX)	4/10/86";
#endif lint
...
void mst_print()
{
	...
	switch (b)
	{
	     ...
	     case 0x6 : printf("6 = Kernel mode Halt\n");
			break;
	     case 0x7 : printf("7 = SCB vector with <1:0> = \n");
			break;
	     case 0x8 : printf("8 = SCB vector with <1:0> = but WCS not loaded\n");
			break;
	...
	printf("Console PROM Revision\t%d\n",getbyte());

	printf("EMM PROM Revision\t%d\n",getbyte());

	printf("Minimun\tHW Revision\t%d.%d\n",getbyte(),getbyte());

	printf("EBox\tUcode Revision\t%d.%d\n",getbyte(),getbyte());

	printf("IBox\tUcode Revision\t%d.%d\n",getbyte(),getbyte());

	printf("Mbox\tUcode Revision\t%d.%d\n",getbyte(),getbyte());

	printf("FBoxA\tUcode Revision\t%d.%d\n",getbyte(),getbyte());

	printf("FBoxM\tUcode Revision\t%d.%d\n",getbyte(),getbyte());

	printf("ACCESS\tUcode Revision\t%d.%d\n",getbyte(),getbyte());

	printf("CPR\tUcode Revision\t%d.%d\n",getbyte(),getbyte());

	printf("MCF\tUcode Revision\t%d.%d\n",getbyte(),getbyte());

	printf("CONTEXT\tUcode Revision\t%d.%d\n",getbyte(),getbyte());

	printf("Ucode Release Revision\t%d.%d\n",getbyte(),getbyte());
	...
	if (rec_length > 0x42)
	{
	  ...
	  printf("Console Software Version\t%d.%d\n",getbyte(),getbyte());
	}

	if (rec_length > 0x44)
	{
	  b = getbyte();
	  printf("CPU Clock Status\t\t%02x%02x\n",getbyte(), b);
	}
