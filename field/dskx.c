...
#ifndef lint
static	char	*sccsid = "@(#)dskx.c	1.8		12/4/86";
#endif lint
...
int seekmask;
	...
	/* set up seek mask */
	if ((seekmask = seekmsk(&pt.pt_part[i].pi_nblocks)) == 0) {
		sprintf(errbuf,"seekmsk function failed");
		report(DR_WRITE,MODULE,errbuf);
		return(1);
	}
	if (dskdebug)
...
struct fs sprblk;			/* super block */
	...
	if (ioctl(fd,DIOCGETPT,&pt) < 0) {
		sprintf(errbuf,"Warning: get pt table ioctl failed: %s\n",
			sys_errlist[errno]);
		report(DR_WRITE,MODULE,errbuf);
		return(1);
	}
	...
	if ((request & 0xff) != 0xff) {
		l = (*partid -1) & 0x07;
		for (i = 0; i < MAXPART; i++) {
		    if (((pt.pt_part[i].pi_blkoff >= pt.pt_part[l].pi_blkoff) &&
		         (pt.pt_part[i].pi_blkoff <= (pt.pt_part[l].pi_blkoff +
		          pt.pt_part[l].pi_nblocks))) ||
		        ((pt.pt_part[l].pi_blkoff >= pt.pt_part[i].pi_blkoff) &&
		         (pt.pt_part[l].pi_blkoff <= (pt.pt_part[i].pi_blkoff +
	...
	for (i = 0; i < MAXPART; i++) {
		*partid = i + 0x61;
		...
		eofprt = (st.st_mode & S_IFMT) == S_IFBLK ?
		    ((pt.pt_part[i].pi_nblocks - 1) - BLKDEV_IOSIZE/DEV_BSIZE) *
		    DEV_BSIZE : (pt.pt_part[i].pi_nblocks - 1) * DEV_BSIZE;
...
int *nblocks;
{
	...
	if (*nblocks < 1024)
		seekmask = 0x3ff;
	else if (*nblocks < 16384)
		seekmask = 0x3fff;
	else if (*nblocks < 32768)
		seekmask = 0x7fff;
	else if (*nblocks < 65536)
		seekmask = 0xffff;
	else if (*nblocks < 131072) 
		seekmask = 0x1ffff;
	else if (*nblocks < 262144)
		seekmask = 0x3ffff;
	else if (*nblocks < 524288)
		seekmask = 0x7ffff;
	else if (*nblocks < 1048576)
		seekmask = 0xfffff;
	return(seekmask);
}
