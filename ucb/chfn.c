#ifndef lint
static char *sccsid = "@(#)chfn.c	1.4 (Ultrix) 1/29/87";
#endif lint
...
/*
    ...
    Collection:	finger
    Author:	Berkeley
    Date:	@(#)chfn.c	4.6 (Berkeley) 7/10/83

    Change finger entries in /etc/passwd
 ...
 01	03-May-85, Greg Tarsa
	...
	Largely based on work done by Fred Avolio, Software Services.

 02	05-Jan-87, Joe Amato
	Added local getpwent(), getpwnam(), getpwuid(), 
	endpwent(), pwskip() functions so as not to use
	yellow pages.

 03 29-Jan-87, Joe Amato
	added saveline[] due to yellow pages special chars
	in passwd file would cause chfn to leave entries
	like +::0:0:: giving anyone served from yellow pages
	a uid and gid of 0.
	...
	char *answer;
{
	...
	do {
		...
		printf("Room number (Exs: 597E or 197C) [%s]: ",
			defaults->office_num);
#else BERKELEY
		printf("Office or Mail Stop (no colons) [%s]: ",
			defaults->office_num);
#endif BERKELEY
		(void) fgets(in_str, BUFSIZ, stdin);
...
|| illegal_building(in_str)
...
#else BERKELEY
		printf("Office Phone (Ex: 1234 or 123-4567) [%s]: ",
			defaults->office_phone);
#endif BERKELEY
		(void) fgets(in_str, BUFSIZ, stdin);
	 ...
	 */
	do {
		printf("Home Phone (Ex: 123-4567 or 123-456-7890) [%s]: ",
			defaults->home_phone);
		(void) fgets(in_str, BUFSIZ, stdin);
...
static struct passwd passwd_str;
...
getpwent()
{
	...
	passwd_str.pw_name = p;
	p = pwskip(p);
	passwd_str.pw_passwd = p;
	p = pwskip(p);
	passwd_str.pw_uid = atoi(p);
	p = pwskip(p);
	passwd_str.pw_gid = atoi(p);
	passwd_str.pw_quota = 0;
	passwd_str.pw_comment = EMPTY;
	p = pwskip(p);
	passwd_str.pw_gecos = p;
	p = pwskip(p);
	passwd_str.pw_dir = p;
	p = pwskip(p);
	passwd_str.pw_shell = p;
	while(*p && *p != '\n') p++;
	*p = '\0';
	return(&passwd_str);
}
