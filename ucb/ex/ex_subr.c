...
#ifndef lint
static char *sccsid = "@(#)ex_subr.c	1.3	ULTRIX	10/3/86";

/*
Original SCCS ID:
	 "@(#)ex_subr.c	7.10 (Berkeley) 6/7/85
*/
#endif not lint
/*
    ex_subr.c
 ...
 ~~~~~~~~~~~~~~~~~~~
01	28-May-85, Greg Tarsa
	SPR: ICA-333. Fixed a problem where onsusp() would
	improperly refresh the screen upon restart when the
	cursor was in the continuation portion of a continued
	line.  Caused "Internal error: vgoto" when only one
	continued line in screen.
...
onsusp()
{
	...
	f = setty(normf);	/* restore original tty modes */
	vnfl();
	putpad(TE);
	flush();
	...
	if (!inopen)
		...
		/* Mark all lines as dirty for next sync/redraw */
		vdirty(0, LINES);

/* 		GT01: remove.  Not necessary and causes major
		problems with continued lines.
