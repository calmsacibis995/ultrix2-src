#ifndef lint
static	char	*sccsid = "@(#)fasl.c	1.2	(ULTRIX)	7/15/85";
#endif lint
 ...
 * 001	Modify to work correctly with dynamically allocated FILE
 *	structures.
 *
 *	Based on:  fasl.c,v 1.8 83/09/12 14:17:38 sklower Exp $
 *
 ************************************************************************/
...
#define offset(p) (((p).r_pcrel) ? ((int) code_core_org): 0)
	...
	/* Set up a fake port so we can read from core.  Make it look like
	 * a string as in sprintf().  Set to start at beginning of lit.
	 */
	incore_file._flag = _IOREAD | _IOSTRG;
	incore_file._base = incore_file._ptr = (char *) literal_core_org;
	incore_file._cnt = lit_end - lit_org;

	if(debug)printf("lit_org %d, charstrt  %d\n",lit_org, incore_file._base);
	...
	while (linktab < (lispval *)lc_end)
	{
	   ...
	   uctolc = ouctolc;
	   getc(&incore_file);			/* eat trailing blank */
	   if(debugmode != nil)
	...
	*/
	...
	for( ; curbind->b_type != -1 ; curbind++) 
	{
	    ...
	    getc(&incore_file);		/* eat trailing null */
	    protect(rdform);
