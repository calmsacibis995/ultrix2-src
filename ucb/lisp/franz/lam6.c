#ifndef lint
static	char	*sccsid = "@(#)lam6.c	1.2	(ULTRIX)	7/15/85";
#endif lint
 ...
 * 001	Modify to work with dynamically allocated FILE structures.
 *
 *	Based on:  lam6.c,v 1.5 83/09/12 14:16:37 sklower
 *
 ************************************************************************/
...
Lreadli()
{
	...
	for(work = lbot->val; TYPE(work)==DTPR; work=work->d.cdr)
		count++;
	string = alloca(count);
	dummy_port._flag = _IOSTRG | _IOREAD;
	dummy_port._cnt = count -1;
	dummy_port._base = string;
	dummy_port._ptr = string;
	dummy_port._file = -1;
	for(work = lbot->val; TYPE(work)==DTPR; work=work->d.cdr) {
		handy = work->d.car;
		switch(TYPE(handy)) {
