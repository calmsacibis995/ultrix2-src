#ifndef lint
static	char	*sccsid = "@(#)grep.c	1.2	(ULTRIX)	9/30/85";
#endif lint
...
char *ebp, *cbp;	/* need cbp global for blkno calculation */
execute(file)
{
	register char *p1, *p2;
	register c;
...
succeed(f)
{
	...
	if (bflag) {
		...
		if (blkno%BUFSIZ == 0) subscript = BUFSIZ;
		else subscript = blkno % BUFSIZ; /* handle partial blks */
		printf("%d:",(blkno - (&ibuf[subscript] - cbp)
				- strlen(linebuf))/512);
	}
	if (nflag)
		printf("%ld:", lnum);
