#ifndef lint
static	char	*sccsid = "@(#)collect.c	1.6	(ULTRIX)	12/10/84";
#endif lint
 ...
 * collect.c
 *
 *	sccsid[] = "@(#)collect.c	2.15 (Berkeley) 8/11/83";
 *
 *	03-Jan-84	mah.  Added prompt for message text.
 *
 *	10-Jan-84	mah.  Suppress new line between message header
 *			      and prompt for message text.
 */
	...
	struct header *hp;
{
	...
	struct tchars special;		/*added for 3-Jan-84 change*/

	noreset++;			/*string resets suspended*/
	ibuf = obuf = NULL;
	if (value("ignore") != NOSTR)
		hf = 1;			/*ignore interrupts*/
	else
		hf = 0;			/*don't ignore interrupts*/
	hadintr = 0;
# ifdef VMUNIX
					/*setup signal handling routines*/
...
# endif VMUNIX
	newi = NULL;			/*set file pointers to null*/
	newo = NULL;
					/*open file for write*/
	...
	files[fileno(obuf)].flags = LIB_OPEN & ~KEEP_OPEN;
	newo = obuf;			/*set file ptr to write file*/
	...
	newi = ibuf;			/*set file ptr to read file*/
	remove(tempMail);		/*unlink file (insure privacy)*/
	...
	t = GTO|GSUBJECT|GCC|GNL;	/*set t to indicate - grab To:,
					 *Subject:,and Cc: lines; and
					 *"print blank line after".
					 */
	c = 0;
	if (intty && sflag == NOSTR && hp->h_subject == NOSTR && value("ask"))
					/*if this is a terminal and no subject
					 *has been specified and "ask" is true,
					 *then negate "print blank line after"
					 *and set c.
					 */
		t &= ~GNL, c++;
					/*added 10-Jan-84*/
	if (intty && value("msgprompt"))
		t &= ~GNL;		/*if msgprompt is set, we never want
					 *to put out a newline.
					 */
					/*end addition*/
	if (hp->h_seq != 0) {
		puthead(hp, stdout, t);
	...
	if (c)
		grabh(hp, GSUBJECT);	/*handles subject line*/
					/*added 1-Jan-84 mah*/
	if (intty && value("msgprompt")) {
		...
		if (value("ignoreeof") == NOSTR && value("dot") != NOSTR)
					/*not ignoring eof and dot is in use,
					 *therefore can use either.
					 */
	...
	escape = ESCAPE;		/*set escape value to default (~)*/
					/*if other escape character was
					 *specified reset to that character.
					 */
...
# endif VMUNIX
		flush();
					/*if lenght = 0 then eof*/
		if (readline(stdin, linebuf) <= 0) {
					/*if ignoreeof is set, then will tell
					 *user 35 times to use "."; after that
					 *it takes the eof.
					 */
		...
		case 'r':
			...
			files[fileno(fbuf)].filep = fbuf;
			files[fileno(fbuf)].flags = LIB_OPEN & ~KEEP_OPEN;
			printf("\"%s\" ", cp);
			flush();
			lc = 0;
			cc = 0;
	...
	FILE *ibuf;
{
	...
	files[fileno(of)].filep = of;
	files[fileno(of)].flags = LIB_OPEN & ~KEEP_OPEN;
	lc = 0;
	cc = 0;
	...
	char cmd[];
{
	...
	files[fileno(no)].filep = no;
	files[fileno(no)].flags = LIB_OPEN & ~KEEP_OPEN;
...
collrub(s)
{
	...
	if ((dbuf = fopen(deadletter, "w")) == NULL)
		goto done;
	files[fileno(dbuf)].filep = dbuf;
	files[fileno(dbuf)].flags = LIB_OPEN & ~KEEP_OPEN;
