#ifndef lint
static	char	*sccsid = "@(#)aux.c	1.4	(ULTRIX)	12/10/84";
#endif lint
 ...
 * aux.c
 *
 *	sccsid[] = "@(#)aux.c	2.11 (Berkeley) 8/11/83";
 *
 *	17-Feb-84	mah. Added comments and copyright.
 *
 *	02-Nov-84	ma.  Change the skinning of arpa net addresses to
 *			handle imbedded parentheses.
 *
 */
	...
	struct message *mp;
{
	...
	ibuf = setinput(mp);
	if ((lc = mp->m_lines) <= 0)		/*lc = # lines in msg*/
		return(NOSTR);
	...
	char linebuf[], field[];
{
	...
	if ((cp = index(linebuf, ':')) == NOSTR)/*search for ':';if none,then*/
		return(0);			/* not a header line so return*/
	if (cp == linebuf)			/*if linebuf = 0, then no line*/
		return(0);			/* so return*/
	cp--;					/*backup pointer*/
	while (cp > linebuf && isspace(*cp))	/*backup pointer to char*/
		cp--;				/* preceeding :*/
	c = *++cp;				/*c = char following space*/
	*cp = 0;
	if (icequal(linebuf ,field)) {		/*now search for field*/
		*cp = c;
		return(1);
	}
	*cp = c;
	...
	char name[];
{
	...
	files[fileno(fi)].filep = fi;
	files[fileno(fi)].flags = LIB_OPEN & (~KEEP_OPEN);
	...
	char name[];
{
	...
	if ((f = open(name, 0)) < 0)
		return(0);
	close(f);		/* nothing needs to be done here with files */
	...
	char name[];
{
	...
	if ((f = open(name, 0)) < 0)
		return;
	files[f].filed = f;
	files[f].flags = SYS_OPEN & ~KEEP_OPEN;
	read(f, &w, 1);
	exit(0);
#endif
}
