#ifndef lint
static	char	*sccsid = "@(#)send.c	1.4	(ULTRIX)	12/10/84";
#endif lint
 ...
 *	14-Aug-84	gpc.  Fixed ULTRIX11 header handling.
 *
 */
	...
	if (pid == 0) {
		...
		if (!stat(POSTAGE, &sbuf))
			if ((postage = fopen(POSTAGE, "a")) != NULL) {
				files[fileno(postage)].filep = postage;
				files[fileno(postage)].flags = LIB_OPEN & ~KEEP_OPEN;
	...
	FILE *fi;
{
	...
	files[fileno(nfo)].filep = nfo;
	files[fileno(nfo)].flags = LIB_OPEN & ~KEEP_OPEN;
	if ((nfi = fopen(tempMail, "r")) == NULL) {
		perror(tempMail);
		fclose(nfo);
		return(fi);
	}
	files[fileno(nfi)].filep = nfi;
	files[fileno(nfi)].flags = LIB_OPEN & ~KEEP_OPEN;
	remove(tempMail);
	...
	FILE *fi;
{
	...
	files[fileno(fo)].filep = fo;
	files[fileno(fo)].flags = LIB_OPEN & ~KEEP_OPEN;
	time(&now);
	n = rflag;
	if (n == NOSTR)
		n = myname;
