#ifndef lint
static	char	*sccsid = "@(#)list.c	1.4	(ULTRIX)	12/16/86";
#endif lint
	...
	char **sp;
{
	...
	if (isdigit(nc)) {
		lexnumber = 0;
		while (isdigit(nc)) {
			lexnumber = lexnumber*10 + nc - '0';
			*ncp2++ = nc;
			nc = *ncp++;
		}
		
		/* If the next character is an alpha, assume this
		 * is some strange user name beginning with one
		 * or more digits. Fix for spr +--> ICA-2167 .
		 */
		if (!isalpha(nc)) {
			*ncp2 = '\0';
			*sp = --ncp;
