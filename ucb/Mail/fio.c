#ifndef lint
static	char	*sccsid = "@(#)fio.c	@(#)fio.c	1.6	(ULTRIX)	3/31/86";
#endif lint
 ...
 * fio.c
 *
 *	sccsid[] = "@(#)fio.c	2.16 (Berkeley) 8/11/83";
 *
	...
	char *linebuf;
{
	...
	*cp = 0;			/*terminates line*/
	...
	return(cp - linebuf + 1);	/*return length of line (-newline)*/
}
...
edstop()
{
	...
	if (Tflag != NOSTR) {
		if ((readstat = fopen(Tflag, "w")) == NULL)
			Tflag = NOSTR;
		files[fileno(readstat)].filep = readstat;
		files[fileno(readstat)].flags = LIB_OPEN & ~KEEP_OPEN;
	}
	...
	char *mode;
{
	...
	files[f].filed = f;
	files[f].flags = files[fildes].flags;
