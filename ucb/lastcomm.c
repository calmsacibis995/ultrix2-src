#ifndef lint
static char *sccsid = "@(#)lastcomm.c	1.3	(ULTRIX)	12/9/85";
#endif
...
#define NUMRECSPERBLOCK	(8192/sizeof(struct acct))
#define BUFSIZE		(sizeof(struct acct)*NUMRECSPERBLOCK)
struct	acct buf[NUMRECSPERBLOCK];
/* we seek by records in a 8192 block = BUFSIZE bytes */
...
main(argc, argv)
	char *argv[];
{
	register int byteloc, cc, readsize = BUFSIZE;
	...
	fstat(fd, &sb);
	if (sb.st_size <= 0) exit(0);
	if (sb.st_size < readsize) {
		byteloc = 0;
		/* round to 'even number of record' bytes */
		readsize = (sb.st_size/sizeof(struct acct))*sizeof(struct acct);
	}
	else byteloc = sb.st_size - readsize;
	while (byteloc >= 0) {
		lseek(fd, byteloc, L_SET);
		cc = read(fd, buf, readsize);
		if (cc <= 0) {
			perror("read");
			break;
		}
		...
		for (; acp >= buf; acp--) {
			register char *cp;
			double x = acp->ac_utime + acp->ac_stime;
			...
			printf("%-*s %s %-*s %-*s %5.2lf secs %.16s\n",
				...
				x, ctime(&acp->ac_btime));
		}
		if (byteloc >= BUFSIZE) byteloc -= BUFSIZE;
		else if (byteloc == 0) exit(0);
		else {	/* pick up the last piece */
			readsize = byteloc;
			byteloc = 0;
		}
	}
