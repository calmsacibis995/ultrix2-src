#ifndef lint
static char *sccsid = "@(#)finger.c	1.5	ULTRIX	10/3/86";
#endif
/*
    Program:	finger.c
    Author:	Berkeley
    Date:	@(#)finger.c	4.5 (Berkeley) 9/16/83
    Collection:	finger

    This is a finger program.  It prints out useful information
    about users by digging it up from various system files.  It
    is not very portable because the most useful parts of the
    information (the full user name, office, and phone numbers)
    are all stored in the VAX-unused gecos field of /etc/passwd,
    which, unfortunately, other UNIXes use for other things.

    There are three output formats, all of which give login
    name, teletype line number, and login time.  The short
    output format is reminiscent of finger on ITS, and gives one
    line of information per user containing in addition to the
    minimum basic requirements (MBR), the full name of the user,
    his idle time and office location and phone number.  The
    quick style output is UNIX who-like, giving only name,
    teletype and login time.  Finally, the long style output
    give the same information as the short (in more legible
    format), the home directory and shell of the user, and, if
    it exits, a copy of the file .plan in the users home
    directory.  Finger may be called with or without a list of
    people to finger -- if no list is given, all the people
    currently logged in are fingered.
    ...
    The program is validly called by one of the following:
      ...
      finger -m username {match against user name field only}
      finger -h		{exclude .project in long listings}
      finger -p		{exclude .plan in long listings}
      finger -f 	{disables printing of headers in short/quick listings}

  where 'namelist' is a list of users login names.
    The other options can all be given after one '-', or
    each can have its own '-'.

 ------------
 Modification History
 ~~~~~~~~~~~~~~~~~~~
 04	9-Jun-86, Greg Tarsa
	Altered comments, fixed a problem where 4 digit phone numbers
	were not being turned into 'x' extensions.  Fixed a buffer
	allocation problem.  Fixed problem where office phone was ending
	up as home phone under certain circumstances.

 03	23-May-86, Greg Tarsa
	Replaced the kludgy and baroque phone() function with a clearer
	and more elegant function.

 02	20-May-86, Greg Tarsa
	More changes from Fred Avolio.  Fix to problem where #'s
	greater than 4 digits were not being treated as
	extensions.  Added a format for 5 digit numbers. Fixed
	problem with occasional core dumps.

 01	03-May-85, Greg Tarsa
	Conditionalized Berkeley office code to make more general.
	Define BERKELEY to get it back.
	Changes largely based on work done by Fred Avolio, Software
	Services.
...
#define		MAXPHONESIZE	20	/* for phone number assembly */
 ...
 *  phone
 *
 *  Take the specified phone number and format it according to the
 *  following template:
 *
 *		11          
 *		109876543210	  
 *		xxx-xxx-xxxx\0
 *
 *  The pattern is filled in from the left so that any positions not
 *  filled are left out.
 *
 * for reference: s[len] is the trailing null.
 */
...
{
	char		*strsave();
	char		fonebuf[MAXPHONESIZE];
	char		*cp;
	int		i, pos;

    /* fill in fonebuf from the left, putting in dashes where necessary */
    for (cp = &fonebuf[MAXPHONESIZE-1], pos = -1; len >= 0; len--, pos++)
	{
	/* Put dashes in for long extension or full phone */
	if (pos == 4 || pos == 8)
	    {
	    *cp-- = '-';
	    pos++;
	...
	*cp-- = s[len];
    ...
    /* put an 'x' in a four-digit extension */
    if (pos == 4)	
	*cp = 'x';
    else
	cp++;		/* or position to end of string */
...

{
	...
	if(  pwdt != NILPWD )  {
	    ...
	    while(  (*gp != NULL)  &&  (*gp != COMMA)  )  {	/* name */
	        /* process '&' in name to be same as login name */
		if( *gp ==SAMENAME )  {
		    lp = pwdt->pw_name;
		    *bp++ = CAPITALIZE(*lp++);
		    while( *lp != NULL )  {
		...
		else  {			/* or just copy raw name */
		    *bp++ = *gp;
		}
		gp++;
	    }
	    *bp = NULL;			/* Null terminate the name */
	    pers->realname = malloc( strlen( &buffer[0] ) + 1 );
	    strcpy( pers->realname, &buffer[0] );

	    /* Process the Office string */
	    if( *gp++ == COMMA )  {
		alldigits = 1;
		bp = &buffer[ 0 ];
