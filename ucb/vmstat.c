#ifndef lint
static char *sccsid = "@(#)vmstat.c	1.3	ULTRIX	10/3/86";
#endif lint
 ...
 *
 * 27-May-86 -- jrs
 *	Adjust interrupt count for clocks in multiple cpu systems
 *
 *	Derived from 4.2BSD labelled:
 *		@(#)vmstat.c	4.11 (Berkeley) 9/25/83
 *
 *-----------------------------------------------------------------------
 */
	...
	char **argv;
{
	...
	lseek(mf, (long)nl[X_ACTIVE].n_value, 0);
...
#endif
#ifdef vax
	printf("%4d%4d", (rate.v_intr/nintv) - (activecpu * hz),
