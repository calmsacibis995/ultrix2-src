 ...
 *	      (vjh, April 29, 1986)					*
 *									*
 *	003 - Fixed bug in down().  Added test for jsb routine when	*
 *	      determining the start address of a function in		*
 *	      findbeginning().						*
 *	      (vjh, August 14, 1985)					*
 *									*
 *	002 - Pushretval() now returns a boolean value.  Returns false	*
 *	      when value is inaccessible; used to panic.		*
 *	      (vjh, July 18, 1985)					*
 *									*
 *	001 - Fixed internal error - unexpected nil frame; occured when *
 *	      pc pointed to last instruction in main routine - 		*
 *	      getcurframe() was returning reg[PROGCTR]+1 as save_pc, 	*
 *	      which in this case is in hyper-space. Modified 		*
 *	      walkstack():  fixed dependencies on the above; removed	*
 *	      the mysterious $b# from stack traces.			*
 *	      (Victoria Holt, April 23, 1985)				*
 *									*
 ************************************************************************/
...
public Boolean pushretval(len, isindirect)
Integer len;
Boolean isindirect;
{
    Word r0;
...
Boolean dumpvariables;
{
	...
	getcurfunc(frp, &f);
	do {
	    while (isinline(f)) {	/* Eliminates display of "$b#" */
	        f = container(f);	/* unnamed blocks */
	    }
	    printcallinfo(f, frp);
	    if (dumpvariables) {
		dumpvars(f, frp);
