 ...
 *	      (vjh, August 23, 1985)					*
 *									*
 *	001 - Added saved-command feature:  <cr> alone on a command 	*
 *	      line reexecutes previously entered command.		*
...
private char prevbuf[MAXLINESIZE];
...
public Token yylex()
{
	...
	if (line == nil) {
	    ...
	    if (*p == '\n') {
	        strcpy(scanner_linebuf, prevbuf);
		p = scanner_linebuf;
	    } else {
	        strcpy(prevbuf, p);
	    }
