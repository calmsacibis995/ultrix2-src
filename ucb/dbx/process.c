...
/*
 * Resume execution up to the given address.
 *
 *	>> It is assumed that
 ...
 *
 * It turns out that this is a bad assumption.  The code has been modified
 * so that bkpts. are set/unset each time this routine is called.
 */
...
Address addr;
{
    ...
    if (addr != curpc) {
        ...
        /* Single step the machine before setting breakpoints in case
     	 * we are already stopped for a breakpoint; don't want to execute
     	 * the same bkpt. instruction again.
     	 */
